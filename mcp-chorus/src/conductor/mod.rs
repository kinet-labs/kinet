...
impl<A> fmt::Debug for KinetConductor<A>
where
    A: Acs<Timestamp>,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("KinetConductor")
...
impl<A> Conductor for KinetConductor<A>
where
    A: Acs<Timestamp>,
{
...
mod tests {
    ...
    fn conductor() -> KinetConductor<NopAcs<Timestamp>> {
        let mut conductor = KinetConductor::genesis(config(), ()).unwrap();
        assert!(matches!(
            conductor.poll(),
    ...
    fn drain_outputs<A: Acs<Timestamp>>(
        conductor: &mut KinetConductor<A>,
    ) -> Vec<ConductorOutput<KinetConductor<A>>> {
    ...
    fn complete_slot<A: Acs<Timestamp>>(
        ...
        slot: u64,
        time: u64,
    ) -> Result<Vec<ConductorOutput<KinetConductor<A>>>, ConductorError> {
    ...
    fn extract_open_slots<A: Acs<Timestamp>>(
        outputs: &[ConductorOutput<KinetConductor<A>>],
    ...
    fn extract_runtime_open_slots<A: Acs<Timestamp>>(
        output: ConductorOutput<KinetConductor<A>>,
    ) -> BTreeMap<Slot, Timestamp> {
        match output {
    ...
    fn genesis_opens_exactly_one_window() {
        let mut conductor = KinetConductor::<NopAcs<Timestamp>>::genesis(config(), ()).unwrap();
    ...
    fn runtime_finalization_opens_next_window_without_closing_slots() {
        ...
        .unwrap();
        let mut conductor =
            KinetConductor::<NopAcs<Timestamp>>::genesis(runtime_config, ()).unwrap();
    ...
    fn deadline_decision_opens_next_window_and_derives_first_slot() {
        let mut conductor = KinetConductor::<MessageAcs>::genesis(config(), ()).unwrap();
        let _ = conductor.poll().unwrap();
        for slot in 0..=7 {
            assert!(complete_slot(&mut conductor, slot, 100).unwrap().is_empty());
        }
    ...
    fn bounded_open_invariant_holds_in_stalled_simulation() {
        ...
        let mut conductor =
            KinetConductor::<NopAcs<Timestamp>>::genesis(config.clone(), ()).unwrap();
        let _ = conductor.poll().unwrap();
