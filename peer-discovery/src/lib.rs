...
use kinet_executor_glue::{PeerEntry, PeerEntryAddress};
use kinet_node_config::NodeBootstrapPeerConfig;
...
#[derive(Debug, Clone)]
pub struct PeerSource<PK: kinet_crypto::certificate_signature::PubKey> {
    pub id: NodeId<PK>,
    pub addr: SocketAddr,
}
...
pub struct KinetNameRecord<ST: CertificateSignatureRecoverable> {
    pub name_record: NameRecord,
    pub signature: ST,
}

impl<ST: CertificateSignatureRecoverable> KinetNameRecord<ST> {
    ...
    ) -> KinetNameRecordWithPubkey<'_, ST> {
        KinetNameRecordWithPubkey {
            record: self,
            pubkey,
        }
    }
...
impl<ST: CertificateSignatureRecoverable> TryFrom<&PeerEntry<ST>> for KinetNameRecord<ST> {
...
impl<ST: CertificateSignatureRecoverable> TryFrom<&KinetNameRecord<ST>> for PeerEntry<ST> {
    type Error = <ST as CertificateSignature>::Error;

    fn try_from(record: &KinetNameRecord<ST>) -> Result<Self, Self::Error> {
...
impl<ST: CertificateSignatureRecoverable> From<KinetNameRecordWithPubkey<'_, ST>>
    for NodeBootstrapPeerConfig<ST>
{
    fn from(record_with_pubkey: KinetNameRecordWithPubkey<'_, ST>) -> Self {
...
pub struct KinetNameRecordWithPubkey<'a, ST: CertificateSignatureRecoverable> {
    record: &'a KinetNameRecord<ST>,
    pubkey: CertificateSignaturePubKey<ST>,
}
...
pub trait PeerDiscoveryAlgo {
    ...
    fn get_name_records(
        ...
        KinetNameRecord<Self::SignatureType>,
    >;

    fn get_name_record(
        &self,
        id: &NodeId<CertificateSignaturePubKey<Self::SignatureType>>,
    ) -> Option<&KinetNameRecord<Self::SignatureType>>;
}
...
mod tests {
    ...
    fn test_name_record_with_unknown_ports_and_capabilities() {
        ...
        let signed_record = KinetNameRecord::<SecpSignature> {
            name_record: decoded,
            signature,
        };
    ...
    fn test_name_record_roundtrip() {
        ...
        let decoded_signed =
            KinetNameRecord::<SecpSignature>::decode(&mut signed_encoded.as_slice()).unwrap();
    ...
    fn test_peer_entry_to_kinet_name_record_invalid_signature() {
        ...
        let result = KinetNameRecord::<SecpSignature>::try_from(&peer_entry);
        assert!(result.is_err());
    }

    #[test]
    fn test_peer_entry_kinet_name_record_roundtrip_with_auth() {
        ...
        let original_kinet_record = KinetNameRecord::<SecpSignature>::new(name_record, &keypair);

        let pubkey = original_kinet_record.recover_pubkey().unwrap().pubkey();
        let peer_entry = PeerEntry::from(original_kinet_record.with_pubkey(pubkey));
        assert_eq!(peer_entry.auth_port.get(), auth_port);

        let converted_kinet_record =
            KinetNameRecord::<SecpSignature>::try_from(&peer_entry).unwrap();

        assert_eq!(
            converted_kinet_record.name_record,
            original_kinet_record.name_record
        );
        assert_eq!(
            converted_kinet_record.signature,
            original_kinet_record.signature
        );
    }

    #[test]
    fn test_peer_entry_kinet_name_record_roundtrip_with_direct_udp() {
        ...
        assert_eq!(
            converted_kinet_record.name_record.authenticated_udp_port(),
            auth_port
        );
        assert_eq!(
            converted_kinet_record.name_record.direct_udp_port(),
            Some(direct_udp_port)
        );
    }
    ...
    fn test_bootstrap_peer_config_rejects_zero_auth_port() {
        ...
        let record = KinetNameRecord::<SecpSignature>::new(
        ...
        let mut serialized = toml::to_string(&kinet_node_config::NodeBootstrapConfig {
        ...
        let parsed =
            toml::from_str::<kinet_node_config::NodeBootstrapConfig<SecpSignature>>(&serialized);
    ...
    #[test]
    fn test_peer_entry_kinet_name_record_roundtrip_with_encrypted_tcp() {
        ...
        assert_eq!(
            converted_kinet_record.name_record.encrypted_tcp_port(),
            Some(encrypted_tcp_port)
        );
    }
}
