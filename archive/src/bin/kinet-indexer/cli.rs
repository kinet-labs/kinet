...
use kinet_archive::cli::{ArchiveArgs, BlockDataReaderArgs};
    ...
    pub fn parse() -> ParsedCli {
        let mut args = match CliArgs::try_parse() {
            ...
            Err(err) => match err.kind() {
                ...
                _ => {
                    eprintln!("failed to parse kinet-indexer args: {err:?}");
                    process::exit(2);
                }
            },
        };
        ...
        match args.into_cli() {
            ...
            Err(err) => {
                eprintln!("failed to load kinet-indexer configuration: {err:?}");
                process::exit(2);
            }
        }
    }
...
#[derive(Debug, Parser)]
#[command(name = "kinet-indexer", about, long_about = None)]
pub struct CliArgs {
    #[command(subcommand)]
...
mod tests {
    use clap::Parser;
    use kinet_archive::cli::{ArchiveArgs, AwsCliArgs, BlockDataReaderArgs};
    ...
    fn validate_set_start_block_requires_archive_sink() {
        let args = CliArgs::try_parse_from(["kinet-indexer", "set-start-block", "--block", "100"]);
