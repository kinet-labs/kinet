...
/// Runtime configuration for the `kinet-archiver` binary.
///
...
///
/// ```text
/// kinet-archiver --config config.toml --max-blocks-per-iteration 50
/// ```
///
...
impl Cli {
    pub fn parse() -> ParsedCli {
        Self::try_parse().unwrap_or_else(|err| {
            eprintln!("failed to load kinet-archiver configuration: {err:?}");
            process::exit(2);
        })
    }
...
mod tests {
    ...
    fn config_flag_reads_toml_file() {
        ...
        let (_, cli) =
            CliArgs::parse_from(["kinet-archiver", "--config", file.path().to_str().unwrap()])
                .into_cli()
