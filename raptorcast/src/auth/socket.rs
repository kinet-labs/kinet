...
use kinet_dataplane::{UdpSocketHandle, UnicastMsg};
use kinet_executor::{ExecutorMetrics, ExecutorMetricsChain};
use kinet_peer_discovery::NameRecord;
use kinet_peer_score::IdentityScore;
use kinet_types::{NodeId, UdpPriority};
use thiserror::Error;
    ...
    AP: AuthenticationProtocol,
{
    ...
    pub fn segment_size(&self, mtu: u16) -> u16 {
        kinet_dataplane::udp::segment_size_for_mtu(mtu) - AP::HEADER_SIZE
    }
...
mod tests {
    ...
    use bytes::Bytes;
    use futures::poll;
    use kinet_dataplane::{DataplaneBuilder, UnicastMsg};
    use kinet_peer_discovery::NameRecord;
    use kinet_secp::KeyPair;
    use kinet_types::UdpPriority;
    use kinet_wireauth::{Config, DEFAULT_RETRY_ATTEMPTS};
    ...
    impl AuthPacketFramer<kinet_secp::PubKey> for PanicFramer {
        type Decoded = Bytes;
    ...
    struct PeerNode {
        ...
        public_key: kinet_secp::PubKey,
        _tcp_socket: kinet_dataplane::TcpSocketHandle,
        _control: kinet_dataplane::DataplaneControl,
    }
    ...
    impl PeerNode {
        fn new(seed: u8, with_non_authenticated: bool) -> Self {
            ...
            if with_non_authenticated {
                udp_sockets.push((kinet_dataplane::UdpSocketId::Raptorcast, bind_addr));
            }

            let mut dp = DataplaneBuilder::new(1000)
                .with_tcp_sockets([(kinet_dataplane::TcpSocketId::Raptorcast, bind_addr)])
            ...
            let non_auth_addr = non_authenticated_socket
                .as_ref()
                .map(kinet_dataplane::UdpSocketHandle::local_addr);
        ...
        fn connect(&mut self, peer_public: &kinet_secp::PubKey, peer_addr: SocketAddr) {
            self.socket
    ...
    async fn test_framed_socket_rejects_unauthenticated_messages() {
        ...
        let mut send_dp = DataplaneBuilder::new(1000)
            .with_udp_sockets([(kinet_dataplane::UdpSocketId::Raptorcast, bind_addr)])
            .build();
        ...
        let auth_socket = AuthenticatedSocketHandle::new(
            recv_socket,
            NoopAuthProtocol::<kinet_secp::PubKey>::new(),
        );
