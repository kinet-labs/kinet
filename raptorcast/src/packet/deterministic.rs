...
use kinet_dataplane::udp::segment_size_for_mtu;
use kinet_merkle::{MerkleHash, MerkleTree};
use kinet_raptor::Encoder;
use kinet_types::{Epoch, NodeId, Round, ETHERNET_MTU};
use kinet_wireauth::messages::DataPacketHeader;
...
impl PacketLayout {
    ...
    pub fn chunk_hash<PT: PubKey>(&self, chunk: &Chunk<PT>) -> kinet_crypto::hasher::Hash {
        let mut hasher = kinet_crypto::hasher::HasherType::new();
        hasher.update(&chunk.payload()[self.merkle_hashed_range()]);
        kinet_crypto::hasher::Hasher::hash(hasher)
    }
}
...
mod tests {
    ...
    use kinet_crypto::{certificate_signature::CertificateSignaturePubKey, hasher::Hasher as _};
    use kinet_merkle::{MerkleHash, MerkleTree};
    use kinet_raptor::r10::lt::MAX_TRIPLES;
    use kinet_secp::{KeyPair, SecpSignature};
    use kinet_types::{Epoch, NodeId, Round};
    use kinet_validator::validator_set::MAX_VALIDATOR_SET_SIZE;
    use zerocopy::Ref;
