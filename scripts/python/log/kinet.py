...
class BftLog:
    ...
    def received_proposal_df(self):
        df = self.df[
            (self.df['target'] == 'kinet_consensus_state')
    ...
    def received_votes_df(self):
        df = self.df[
            (self.df['target'] == 'kinet_consensus::vote_state')
    ...
    def create_proposal_df(self):
        df = self.df[
            (
                ...
                (self.df['target'].str.startswith('kinet_eth_txpool')) &
                (self.df['message'] == 'created proposal')
            )
        ]
        df = df[['timestamp', 'fields', 'target']]
        df['seq_num'] = df.apply(lambda row: row['fields']['try_propose_seq_num'] if row['target'] == 'kinet_consensus_state' else row['fields']['proposed_seq_num'], axis=1).astype(int)
        df['num_tx'] = df.apply(lambda row: 0 if row['target'] == 'kinet_consensus_state' else row['fields']['proposal_num_tx'], axis=1).astype(int)
