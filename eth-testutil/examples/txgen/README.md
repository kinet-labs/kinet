...
- `kinet-eth-testutil/examples/txgen/cli.rs` – CLI arguments, logging setup, and config loading.
- `kinet-eth-testutil/examples/txgen/sample_configs/` – Minimal JSON/TOML configs showing multiple workload groups.
- `kinet-eth-testutil/examples/txgen/report/` – Optional JSON reporting pipeline that snapshots metrics, Prometheus stats, and RPC responses when `report_dir` is configured.
