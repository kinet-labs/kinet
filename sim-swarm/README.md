...
[`kinet-sim`](../kinet-sim) discrete-event engine. Still protocol-agnostic:
everything here is parameterized only by a node address type `Addr` and a wire
message type `Msg`, with no Kinet or consensus knowledge. Protocol-specific
harnesses sit on top — the Kinet BFT mock-swarm in this repository is one
consumer. See [`docs/design.md`](docs/design.md) for the design, and
[`kinet-sim/docs/design.md`](../kinet-sim/docs/design.md) for the underlying
engine.
...
```rust
use kinet_sim::{time::millis, Ctx, Handle, StepLabel, Time};
use kinet_sim_swarm::{Net, Network, SimClient, Swarm};
