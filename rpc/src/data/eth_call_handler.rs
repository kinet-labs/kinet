...
use dashmap::DashMap;
use kinet_ethcall::{ffi::PoolConfig, KinetExecutor};
...
impl EthCallHandler {
    pub fn new(config: EthCallHandlerConfig, triedb_path: &Path) -> Self {
        let executor = Arc::new(KinetExecutor::new(
            config.pool_low,
            config.pool_high,
            config.pool_block,
...
impl<'a> EthCallPermit<'a> {
    pub async fn execute<T, E, F>(
        mut self,
        f: impl FnOnce(&'a EthCallHandlerConfig, &'a KinetExecutor) -> F,
    ) -> Result<T, E>
    where
