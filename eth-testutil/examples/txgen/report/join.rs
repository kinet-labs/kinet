...
pub async fn join_stats(
    ...
    let specs = [
        CounterSpec {
            metric: "kinet_bft_txpool_create_proposal_elapsed_ns",
            label: "BFT Proposal Creation Latency",
            unit: "ms",
            scale: 1.0 / 1_000_000.0,
            query: format!(
                "quantile(0.5, avg(kinet_bft_txpool_create_proposal_elapsed_ns{{{tags}}}[1m]))"
            ),
        },
        CounterSpec {
            metric: "kinet_state_consensus_events_local_timeout",
            label: "Consensus Local Timeout Events",
            unit: "count",
            scale: 1.0,
            query: format!("max(rate(kinet_state_consensus_events_local_timeout{{{tags}}}[1m]))"),
        },
        CounterSpec {
            metric: "kinet_state_consensus_events_proposal_with_tc",
            label: "Consensus Proposals with TC",
            unit: "count",
            scale: 1.0,
            query: format!(
                "max(rate(kinet_state_consensus_events_proposal_with_tc{{{tags}}}[1m]))"
            ),
        },
        CounterSpec {
            metric: "kinet_state_consensus_events_creating_proposal",
            label: "Consensus Creating Proposal Events",
            unit: "count",
            scale: 1.0,
            query: format!(
                "max(rate(kinet_state_consensus_events_creating_proposal{{{tags}}}[1m]))"
            ),
        },
        CounterSpec {
            metric: "kinet_execution_ledger_num_tx_commits",
            label: "Execution Ledger TX Commits",
            unit: "ops/sec",
            scale: 1.0,
            query: format!(
                "quantile(0.5, rate(kinet_execution_ledger_num_tx_commits{{{tags}}}[1m]))"
            ),
        },
    ];
