...
pub enum ExecutionMessage {
    SyncRequest(ffi::kinet_sync_request),
    SyncUpsert(StateSyncUpsertType, Vec<u8>),
    SyncDone(ffi::kinet_sync_done),
}
...
impl<'a, PT: PubKey> StreamState<'a, PT> {
    ...
    async fn try_queue_response(&mut self) -> Result<(), tokio::io::Error> {
        ...
        self.write_execution_request(ffi::kinet_sync_request {
    ...
    async fn read_execution_message(
        ...
        let execution_msg = match msg_type {
            ffi::kinet_sync_type_SYNC_TYPE_TARGET => {
                panic!("live-mode execution sent unexpected SyncTarget");
            }
            ffi::kinet_sync_type_SYNC_TYPE_REQUEST => {
                let mut buf = [0_u8; std::mem::size_of::<ffi::kinet_sync_request>()];
            ...
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_CODE => {
                self.read_sync_upsert(StateSyncUpsertType::Code).await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_ACCOUNT => {
                self.read_sync_upsert(StateSyncUpsertType::Account).await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_STORAGE => {
                self.read_sync_upsert(StateSyncUpsertType::Storage).await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_ACCOUNT_DELETE => {
                self.read_sync_upsert(StateSyncUpsertType::AccountDelete)
                    .await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_STORAGE_DELETE => {
                self.read_sync_upsert(StateSyncUpsertType::StorageDelete)
                    .await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_UPSERT_HEADER => {
                self.read_sync_upsert(StateSyncUpsertType::Header).await?
            }
            ffi::kinet_sync_type_SYNC_TYPE_DONE => {
                let mut buf = [0_u8; std::mem::size_of::<ffi::kinet_sync_done>()];
                self.stream.read_exact(&mut buf).await?;
    ...
    async fn write_execution_request(
        &mut self,
        request: ffi::kinet_sync_request,
    ) -> Result<(), tokio::io::Error> {
        self.stream
            .write_u8(ffi::kinet_sync_type_SYNC_TYPE_REQUEST)
            .await?;
        let request: [u8; std::mem::size_of::<ffi::kinet_sync_request>()] = unsafe {
            #[allow(clippy::missing_transmute_annotations)]
