...
use futures::Stream;
use kinet_consensus_types::validator_data::{ValidatorSetData, ValidatorSetDataWithEpoch};
use kinet_crypto::certificate_signature::{
    CertificateSignaturePubKey, CertificateSignatureRecoverable,
};
use kinet_executor::{Executor, ExecutorMetrics, ExecutorMetricsChain};
use kinet_executor_glue::{KinetEvent, ValSetCommand};
use kinet_types::{Epoch, ExecutionProtocol, SeqNum};
    ...
    EPT: ExecutionProtocol,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if let Some(next_val_data) = this.next_val_data.take() {
            return Poll::Ready(Some(KinetEvent::ValidatorEvent(
                kinet_executor_glue::ValidatorEvent::<SCT>::UpdateValidators(next_val_data),
            )));
        }
