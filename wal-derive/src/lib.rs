...
pub fn derive_wal_log(input: TokenStream) -> TokenStream {
                ...
                Fields::Named(fields) if fields.named.len() == 1 => {
                    ...
                    quote!(
                        Self::#ident { #field_ident: wal_field } =>
                            ::kinet_wal::wal::WALLog::is_wal_logged(wal_field)
                    )
                }
                Fields::Unnamed(fields) if fields.unnamed.len() == 1 => quote!(
                    Self::#ident(wal_field) => ::kinet_wal::wal::WALLog::is_wal_logged(wal_field)
                ),
                _ => {
    ...
    quote! {
        impl #impl_generics ::kinet_wal::wal::WALLog for #ident #ty_generics #where_clause {
