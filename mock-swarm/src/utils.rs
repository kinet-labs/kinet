...
pub mod test_tool {
    ...
    use kinet_transformer::{LinkMessage, ID};
    use kinet_types::{BlockId, DontCare, Epoch, NodeId, Round, SeqNum};
    ...
    pub fn fake_block(round: Round) -> (ConsensusBlockHeader<ST, SC, EP>, ConsensusBlockBody<EP>) {
        ...
        (
            ConsensusBlockHeader::new(
                ...
                kinet_tfm::base_fee::MIN_BASE_FEE,
                kinet_tfm::base_fee::GENESIS_BASE_FEE_TREND,
                kinet_tfm::base_fee::GENESIS_BASE_FEE_MOMENT,
            ),
            body,
        )
    }
    ...
    pub fn fake_vote_message(kp: &KeyPairType, round: Round) -> VerifiedKinetMessage<ST, SC, EP> {
        let vote = Vote {
            round,
            ..DontCare::dont_care()
        };
    ...
    pub fn fake_timeout_message(kp: &KeyPairType) -> VerifiedKinetMessage<ST, SC, EP> {
    ...
    pub fn fake_request_block_sync() -> VerifiedKinetMessage<ST, SC, EP> {
        let internal_msg = BlockSyncRequestMessage::Headers(BlockRange {
            last_block_id: BlockId(Hash([0x01_u8; 32])),
            num_blocks: SeqNum(1),
        });
        VerifiedKinetMessage::BlockSyncRequest(internal_msg)
    }

    pub fn fake_block_sync() -> VerifiedKinetMessage<ST, SC, EP> {
        ...
        VerifiedKinetMessage::BlockSyncResponse(internal_msg)
    }
}
