...
use kinet_ethcall::{ChainId, KinetExecutor, KinetTracer};
...
use crate::{
    ...
    handlers::{
        debug::{
            ...
            KinetDebugTraceTransactionParams, Tracer, TracerObject,
        },
        parse_ethcall_chain_id, KinetRpcResources,
    },
...
impl From<TracerObject> for KinetTracer {
    fn from(tracer_obj: TracerObject) -> Self {
        match tracer_obj.tracer {
            Tracer::PreStateTracer if tracer_obj.config.diff_mode => KinetTracer::StateDiffTracer,
            Tracer::PreStateTracer => KinetTracer::PreStateTracer,
            Tracer::CallTracer => KinetTracer::CallTracer,
        }
    }
}
...
impl DebugTraceParams for KinetDebugTraceTransactionParams {
    fn requires_replay(&self) -> bool {
...
impl DebugTraceParams for KinetDebugTraceBlockByNumberParams {
    fn requires_replay(&self) -> bool {
        matches!(self.tracer.tracer, Tracer::PreStateTracer)
    }
    fn block_tag_or_hash(&self) -> BlockTagOrHash {
...
impl DebugTraceParams for KinetDebugTraceBlockByHashParams {
    fn requires_replay(&self) -> bool {
        matches!(self.tracer.tracer, Tracer::PreStateTracer)
    }
    fn block_tag_or_hash(&self) -> BlockTagOrHash {
...
pub async fn kinet_debug_trace_replay<T: Triedb>(
    ...
    let tracer: KinetTracer = params.tracer().into();
...
pub async fn collect_debug_trace_via_replay(
    ...
    permit
        .execute(|_, executor| {
            kinet_debug_trace_replay(data_provider, executor, chain_id, max_response_size, params)
        })
        .await
}
