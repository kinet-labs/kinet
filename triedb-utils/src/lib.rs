...
use key::Version;
use kinet_bls::{BlsPubKey, BlsSignatureCollection};
...
use kinet_triedb::TriedbHandle;
pub use kinet_triedb::{MigrationPhase, StorageStats};
use kinet_types::{BlockId, Epoch, Hash, SeqNum, Stake};
...
impl ExecutionStateRead<SecpSignature, BlsSignatureCollection<kinet_secp::PubKey>>
    for TriedbReader
{
    ...
    fn read_valset_at_block(
    ...
    ) -> Vec<(kinet_secp::PubKey, BlsPubKey, Stake)> {
        ...
        for validator_data in validator_set_raw.data() {
            let secp_pubkey =
                kinet_secp::PubKey::from_bytes(validator_data.secp_pubkey.as_slice()).unwrap();
