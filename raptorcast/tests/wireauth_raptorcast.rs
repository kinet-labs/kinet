...
use kinet_executor_glue::{Message, RouterCommand};
use kinet_peer_discovery::{
    driver::PeerDiscoveryDriver, message::Ping, mock::NopDiscovery, KinetNameRecord, NameRecord,
    PeerDiscoveryEvent,
};
use kinet_raptorcast::{create_dataplane_for_tests, DataplaneHandles, RaptorCastEvent};
use kinet_secp::{KeyPair, SecpSignature};
use kinet_types::{Deserializable, Epoch, NodeId, Round, Serializable, Stake};
use rstest::rstest;
...
impl ValidatorInfo {
    ...
    fn create_name_record_with_non_auth_addr(
        ...
        KinetNameRecord::new(name_record, &*self.keypair)
    }
}
...
) -> kinet_raptorcast::config::RaptorCastConfig<SecpSignature> {
    kinet_raptorcast::config::RaptorCastConfig {
        shared_key: keypair,
        mtu: kinet_dataplane::udp::DEFAULT_MTU,
        udp_message_max_age_ms: u64::MAX,
        sig_verification_rate_limit,
        primary_instance: Default::default(),
        secondary_instance: kinet_node_config::FullNodeRaptorCastConfig {
            enable_publisher: false,
            enable_client: false,
            raptor10_fullnode_redundancy_factor: 2f32,
            full_nodes_prioritized: kinet_node_config::FullNodeConfig { identities: vec![] },
            round_span: kinet_types::Round(10),
            invite_lookahead: kinet_types::Round(5),
            max_invite_wait: kinet_types::Round(3),
            deadline_round_dist: kinet_types::Round(3),
            init_empty_round_span: kinet_types::Round(1),
            max_group_size: 10,
            max_num_group: 5,
            invite_future_dist_min: kinet_types::Round(1),
            invite_future_dist_max: kinet_types::Round(5),
            invite_accept_heartbeat_ms: 100,
        },
        deterministic_protocol_rollout: kinet_raptorcast::v1_rollout::CURRENT_STAGE,
    }
}
...
fn create_peer_discovery(
    ...
    std::sync::Mutex<
        kinet_peer_discovery::driver::PeerDiscoveryDriver<
            kinet_peer_discovery::mock::NopDiscovery<SecpSignature>,
        >,
    >,
> {
    let builder = kinet_peer_discovery::mock::NopDiscoveryBuilder {
        known_addresses,
        name_records,
        ..Default::default()
    };
    let pd = kinet_peer_discovery::driver::PeerDiscoveryDriver::new(builder);
...
fn spawn_noop_validator(
    ...
    tokio::task::spawn_local(async move {
        ...
        let mut validator_rc = kinet_raptorcast::RaptorCast::<
            SecpSignature,
            MockMessage,
            MockMessage,
            MockEvent<CertificateSignaturePubKey<SecpSignature>>,
            kinet_peer_discovery::mock::NopDiscovery<SecpSignature>,
            kinet_raptorcast::auth::NoopAuthProtocol<CertificateSignaturePubKey<SecpSignature>>,
            kinet_raptorcast::auth::NopScore<NodeId<CertificateSignaturePubKey<SecpSignature>>>,
        >::new(
            config,
            kinet_raptorcast::raptorcast_secondary::SecondaryRaptorCastModeConfig::None,
            dataplane.tcp_socket,
            (
                dataplane.authenticated_socket,
                kinet_raptorcast::auth::NoopAuthProtocol::new(),
            ),
            None,
...
fn spawn_wireauth_validator(
    ...
    name_records: HashMap<
        ...
        KinetNameRecord<SecpSignature>,
    >,
    peers_to_check: Vec<(SocketAddrV4, kinet_secp::PubKey)>,
    sig_verification_rate_limit: u32,
    ...
    tokio::task::spawn_local(async move {
        ...
        let authenticated = (
            dataplane.authenticated_socket,
            kinet_raptorcast::auth::WireAuthProtocol::new(
                &kinet_raptorcast::auth::metrics::UDP_METRICS,
                wireauth_config.clone(),
                keypair.clone(),
            ),
        );
        let direct_udp = dataplane.direct_udp_socket.map(|socket| {
            let protocol = kinet_raptorcast::auth::WireAuthProtocol::new(
                &kinet_raptorcast::auth::metrics::DIRECT_UDP_METRICS,
                wireauth_config.clone(),
                keypair.clone(),
            );
            (
                socket,
                protocol,
                kinet_raptorcast::auth::NopScore::<
                    NodeId<CertificateSignaturePubKey<SecpSignature>>,
                >::new(),
            )
        });
    ...
    for cmd_tx in cmd_txs {
        cmd_tx
            .send(RouterCommand::AddEpochValidatorSet {
                epoch,
                epoch_start: kinet_types::Round(0),
            ...
            if i != j {
                cmd_tx
                    .send(RouterCommand::Publish {
                        target: kinet_types::RouterTarget::PointToPoint(*node_id),
...
async fn run_test_scenario(
    ...
    match routing_type {
        RoutingType::PointToPoint | RoutingType::DirectPointToPoint => {
            for receiver_idx in 1..NUM_NODES {
                ...
                let target = match routing_type {
                    RoutingType::PointToPoint => kinet_types::RouterTarget::PointToPoint(
                        validator_infos[receiver_idx].nodeid,
                    ),
                    RoutingType::DirectPointToPoint => {
                        kinet_types::RouterTarget::DirectPointToPoint(
                            validator_infos[receiver_idx].nodeid,
                        )
                    }
                    _ => unreachable!(),
        ...
        RoutingType::Raptorcast | RoutingType::Broadcast => {
            ...
            let target = match routing_type {
                RoutingType::Raptorcast => kinet_types::RouterTarget::Raptorcast { round, epoch },
                RoutingType::Broadcast => kinet_types::RouterTarget::Broadcast(epoch),
                _ => unreachable!(),
            };
...
async fn test_rate_limiting_basic() {
    ...
    for i in 0..NUM_MESSAGES {
        ...
        cmd_txs[validator_sender_idx]
            .send(RouterCommand::Publish {
                target: kinet_types::RouterTarget::PointToPoint(
