...
pub fn archive_to_jsonrpc_error<'a, A: Into<std::borrow::Cow<'a, str>>>(
    message: A,
) -> impl FnOnce(kinet_archive::prelude::Report) -> JsonRpcError {
    // Log with debug to get more details, but return a generic error for response
    move |e: kinet_archive::prelude::Report| {
...
impl<T> ArchiveErrorExt<T> for kinet_archive::prelude::Result<T> {
    fn to_jsonrpc_error<'a, A: Into<std::borrow::Cow<'a, str>>>(
        self,
        message: A,
    ) -> JsonRpcResult<T> {
...
impl From<kinet_archive::prelude::Report> for JsonRpcError {
...
impl From<kinet_ethcall::EthTraceError> for JsonRpcError {
    fn from(error: kinet_ethcall::EthTraceError) -> Self {
        use kinet_ethcall::EthTraceError::*;
...
impl From<kinet_ethcall::EthSimulateError> for JsonRpcError {
    fn from(error: kinet_ethcall::EthSimulateError) -> Self {
        use kinet_ethcall::EthSimulateError::*;
        let message = match error {
