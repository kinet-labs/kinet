...
pub struct KinetU256(#[schemars(with = "u128")] pub U256);

impl Serialize for KinetU256 {
...
impl<'de> Deserialize<'de> for KinetU256 {
...
pub struct KinetLog(#[schemars(schema_with = "schema_for_log")] pub Log<LogData>);
...
#[derive(Debug, Serialize, JsonSchema)]
pub struct KinetTransactionReceipt(
...
#[derive(Debug, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct KinetCreateAccessListItem {
...
#[derive(Debug, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct KinetCreateAccessListResult {
    #[schemars(with = "Vec<KinetCreateAccessListItem>")]
...
pub struct KinetFeeHistory(#[schemars(schema_with = "schema_for_fee_history")] pub FeeHistory);
...
impl From<FixedData<32>> for kinet_types::BlockId {
    fn from(value: FixedData<32>) -> Self {
        kinet_types::BlockId(kinet_types::Hash(value.0))
    }
}
...
pub enum SubscriptionResult {
    ...
    KinetNewHeads(KinetNotification<alloy_rpc_types::eth::Header>),
    // Returns all logs with their corresponding commit state.
    KinetLogs(KinetNotification<alloy_rpc_types::eth::Log>),
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct KinetNotification<T> {
...
impl<T> KinetNotification<T> {
    pub(crate) fn map<U>(self, f: impl FnOnce(T) -> U) -> KinetNotification<U> {
        let Self {
            block_id,
            commit_state,
            data,
