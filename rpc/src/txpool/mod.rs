...
use kinet_eth_txpool_types::EthTxPoolIpcTx;
                ...
                result = tx_receiver.recv_async() => {
                    ...
                    for (tx, tx_status_recv_send) in std::iter::once(tx_pair).chain(tx_receiver.drain()) {
                        ...
                        if let Err(e) = ipc_client.feed(EthTxPoolIpcTx::new_with_default_priority(
                            ...
                            warn!("TxPoolBridge IPC feed failed, kinet-bft likely crashed: {}", e);
                        }
                    }

                    if let Err(e) = ipc_client.flush().await {
                        error!("TxPoolBridge IPC flush failed, kinet-bft likely crashed: {}", e);
                    }
                }
...
mod tests {
    ...
    use kinet_eth_testutil::{make_legacy_tx, S1, S2};
    use kinet_eth_txpool_ipc::EthTxPoolIpcStream;
    use kinet_eth_txpool_types::EthTxPoolSnapshot;
