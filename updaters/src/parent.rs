...
use kinet_consensus_types::block::BlockPolicy;
...
impl ParentExecutorMetrics {
    fn record<T>(
        &mut self,
        metric: &'static kinet_executor::MetricDef,
        f: impl FnOnce() -> T,
    ) -> T {
...
pub struct ParentExecutorMetricsGuard<'a> {
    ...
    guard_metric: &'static kinet_executor::MetricDef,
    start: Instant,
}
