...
This directory contains the Kinet setup helper used with the
`category-labs/hive-runner` RPC compatibility tests. It imports the Hive test
chain RLP into Kinet execution, starts `kinet-rpc` against the resulting state,
...
After `kinet-rpc` is running, run the compatibility tests from the
