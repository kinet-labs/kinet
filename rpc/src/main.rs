...
use clap::Parser;
use kinet_archive::archive_reader::{redact_mongo_url, ArchiveReader};
use kinet_event_ring::{EventRing, EventRingPath};
use kinet_node_config::KinetNodeConfig;
...
use kinet_rpc::{
    ...
    event::EventServer,
    handlers::{
        resources::{KinetJsonRootSpanBuilder, KinetRpcResources},
        rpc_handler,
    },
    middleware::{DecompressionGuard, Metrics, TimingMiddleware},
    txpool::EthTxPoolBridge,
    websocket, KINET_RPC_VERSION,
};
use kinet_tracing_timing::TimingsLayer;
...
async fn main() -> std::io::Result<()> {
    ...
    let node_config: KinetNodeConfig = toml::from_str(&std::fs::read_to_string(&args.node_config)?)
    ...
    KINET_RPC_VERSION.map(|v| info!("starting kinet-rpc with version {}", v));
    ...
    rayon::ThreadPoolBuilder::new()
        .thread_name(|i| format!("kinet-rpc-rn-{i}"))
    ...
    let archive_reader = match (&args.mongo_db_name, &args.mongo_url) {
        (Some(db_name), Some(url)) => {
            ...
            match ArchiveReader::init_mongo_reader(
                url.clone(),
                db_name.clone(),
                kinet_archive::prelude::Metrics::none(),
    ...
    let app_state = KinetRpcResources::new(
    ...
    let app = match with_metrics {
        Some(metrics) => HttpServer::new(move || {
            App::new()
                ...
                .wrap(TracingLogger::<KinetJsonRootSpanBuilder>::new())
