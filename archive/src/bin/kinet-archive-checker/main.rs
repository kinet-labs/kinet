...
use kinet_archive::{cli::get_aws_config, prelude::*};
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::builder()
                .with_default_directive("kinet_archive_checker=debug".parse()?)
                .from_env_lossy(),
        )
        .init();
    ...
    info!("Starting kinet-archive-checker with mode: {:?}", args.mode);
    ...
    info!("kinet-archive-checker completed successfully");
    Ok(())
}
