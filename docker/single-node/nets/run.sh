...
usage() {
    ...
    echo "  --toolchain: Execution toolchain basename under kinet-execution/category/core/toolchains (default: gcc-avx2)."
    exit 1
}
...
output_dir="logs"
kinet_bft_root="../.."
devnet_dir="$kinet_bft_root/docker/devnet"
rpc_dir="$kinet_bft_root/docker/rpc"
net_dir="$kinet_bft_root/docker/single-node/nets"
...
export KINET_ROOT=$(realpath "$kinet_bft_root")
...
export GIT_TAG_VERSION=$(git -C "$KINET_ROOT" describe --always)

if [ -z "$CACHED_VOL_ROOT" ] && [ "$USE_PREBUILT" != true ]; then
    toolchain_file="$KINET_EXECUTION_ROOT/category/core/toolchains/${TOOLCHAIN}.cmake"
    if [ ! -f "$toolchain_file" ]; then
        echo "Error: execution toolchain file does not exist: $toolchain_file" >&2
        echo "Set --toolchain to one of:" >&2
        find "$KINET_EXECUTION_ROOT/category/core/toolchains" -maxdepth 1 -type f -name '*.cmake' -exec basename {} .cmake \; | sort >&2
        exit 1
    fi
fi
...
if [ -z "$CACHED_VOL_ROOT" ]; then
    ...
    cp -r "$devnet_dir/kinet/config" "$vol_root/node"
    ...
    if [ "$USE_PREBUILT" = true ]; then
        ...
        docker compose -f compose.yaml -f compose.prebuilt.yaml up build_triedb build_genesis kinet_execution kinet_node kinet_rpc
    else
        # Build kinet execution (needs buildkit so unable to build in docker compose)
        set +e
        ...
        docker build --builder insecure --allow security.insecure \
            -f "$KINET_EXECUTION_ROOT/docker/Dockerfile" \
            --load -t kinet-execution-builder:latest "$KINET_EXECUTION_ROOT" \
        ...
        docker compose up build_triedb build_genesis kinet_execution kinet_node kinet_rpc --build
    fi
    ...
    docker compose up kinet_execution kinet_node kinet_rpc
fi
