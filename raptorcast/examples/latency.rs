...
use kinet_dataplane::DataplaneBuilder;
use kinet_executor::{Executor, ExecutorMetricsChain};
use kinet_executor_glue::{Message, RouterCommand};
use kinet_node_config::{fullnode_raptorcast::FullNodeRaptorCastConfig, FullNodeConfig};
...
use kinet_types::{Deserializable, Epoch, NodeId, Round, RouterTarget, Serializable, Stake};
...
#[derive(Clone, Debug)]
struct MockEvent<P: kinet_crypto::certificate_signature::PubKey> {
    from: NodeId<P>,
    message: MockMessage,
}
...
struct NodeSetup {
    raptorcast: RaptorCast<
        ...
        kinet_raptorcast::auth::WireAuthProtocol,
        kinet_raptorcast::auth::NopScore<NodeId<CertificateSignaturePubKey<SignatureType>>>,
    >,
...
fn setup_node(
    ...
    for participant in participants_to_use {
        ...
        let kinet_name_record =
            KinetNameRecord::<SignatureType>::new(name_record, &participant_keypair);

        routing_info.insert(node_id, kinet_name_record);
    ...
    let mut dataplane = DataplaneBuilder::new(UDP_BW)
        ...
        .with_tcp_sockets([(kinet_dataplane::TcpSocketId::Raptorcast, tcp_addr)])
        .with_udp_sockets([
            (
                kinet_dataplane::UdpSocketId::AuthenticatedRaptorcast,
                authenticated_udp_addr,
            ),
            (
                kinet_dataplane::UdpSocketId::Raptorcast,
    ...
    let tcp_socket = dataplane
        .tcp_sockets
        .take(kinet_dataplane::TcpSocketId::Raptorcast)
        .expect("tcp socket not found");
    let authenticated_socket = dataplane
        .udp_sockets
        .take(kinet_dataplane::UdpSocketId::AuthenticatedRaptorcast)
        .expect("authenticated socket not found");
    let non_authenticated_socket = dataplane
        .udp_sockets
        .take(kinet_dataplane::UdpSocketId::Raptorcast)
    ...
    let auth_protocol = kinet_raptorcast::auth::WireAuthProtocol::new(
