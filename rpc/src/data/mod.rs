...
use kinet_triedb_utils::triedb_env::{BlockKey, FinalizedBlockKey, ProposedBlockKey, Triedb};
...
use crate::{
    ...
    types::{
        eth_json::{BlockTagOrHash, BlockTags, KinetLog, KinetTransactionReceipt},
...
impl From<kinet_archive::prelude::Report> for ChainStateError {
    fn from(e: kinet_archive::prelude::Report) -> Self {
    ...
    T: Triedb,
{
    ...
    pub async fn get_block_receipts(
        &self,
        block: BlockTagOrHash,
    ) -> Result<Vec<KinetTransactionReceipt>, ChainStateError> {
        if let Some(buffer) = &self.buffer {
            if let Some(height) = resolve_block_height_from_buffer(buffer, &block) {
                if let Some(receipts) = buffer.get_receipts_by_block_height(height) {
                    return Ok(receipts.into_iter().map(KinetTransactionReceipt).collect());
                }
            }
        }
    ...
    pub async fn get_logs(
        ...
        if dry_run_get_logs_index {
            if let Some(archive_reader) = self.archive_reader.clone() {
                let non_indexed =
                    HashSet::from_iter(logs.iter().map(|kinet_log| &kinet_log.0).cloned());
...
async fn try_collect_logs_stream_with_heuristic_response_limit<E>(
...
) -> JsonRpcResult<Result<Vec<KinetLog>, E>> {
    ...
    let mut response_logs = Vec::<KinetLog>::default();
...
async fn get_receipts_stream_using_index<'a>(
    ...
    impl Stream<Item = kinet_archive::prelude::Result<(u64, Vec<ReceiptEnvelope<Log>>)>> + 'a,
> {
...
async fn try_create_logs_stream_using_index<'a>(
    ...
    impl Stream<Item = kinet_archive::prelude::Result<(u64, impl Iterator<Item = Log> + 'a)>> + 'a,
> {
    Ok(
...
fn parse_block_content(block_data: HistoricalBlockData, return_full_txns: bool) -> Block {
    ...
    // NOTE: no withdrawals currently in kinet-bft
    Block {
        header: Header {
...
mod tests {
    ...
    use kinet_eth_types::TxEnvelopeWithSender;
