...
`kinet-sim` is the domain-agnostic bottom layer for deterministic protocol
...
timers, and metrics. It knows nothing about Kinet, consensus, or any
particular event or message type.

The engine is written for multiple consumers, present and future. In this
repository, [`kinet-sim-swarm`](../../kinet-sim-swarm) layers a generic
network + swarm harness on top, and the Kinet BFT mock-swarm harness drives
...
types (for the Kinet BFT harness: `KinetEvent`, `Command`) appear only inside
...
the first consumer is low and flat: the Kinet BFT harness measured `W / D`
