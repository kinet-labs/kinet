...
use kinet_event_ring::{DecodedEventRing, EventNextResult};
use kinet_exec_events::{
...
use crate::types::{eth_json::KinetNotification, serialize::JsonSerialized};
...
fn broadcast_block_updates(
    ...
    let block_id = BlockId(kinet_types::Hash(block.start.block_tag.id.bytes));

    let serialized_kinet_header = JsonSerialized::new_shared_with_map(
        KinetNotification {
            block_id,
            commit_state,
    ...
    broadcast_event(
        ...
        EventServerEvent::Block {
            commit_state,
            header: serialized_kinet_header,
...
mod test {
    ...
    use kinet_exec_events::ExecEventDecoder;
    use serde::{de::DeserializeOwned, Serialize};

    use super::*;
    use crate::{
        event::{EventServer, EventServerEvent},
        types::eth_json::KinetNotification,
    };
    ...
    async fn json() {
        ...
        let (commit_state, kinet_header, transactions) = match event {
            EventServerEvent::Gap => {
                panic!("EventServer using snapshot should never produce a gap!")
            }
            EventServerEvent::Block {
        ...
        assert_json::<_, KinetNotification<alloy_rpc_types::Header>>(
            &[&kinet_header],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.kinet-header.json"
            ),
        );
        ...
        assert_json::<_, alloy_rpc_types::Header>(
            &[&kinet_header.data],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.header.json"
            ),
        );

        let (tx_first, tx_first_receipt, tx_first_kinet_logs) = transactions.first().unwrap();

        assert_json::<_, alloy_rpc_types::Transaction>(
            &[&tx_first],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.tx.0.json"
            ),
        );
        ...
        assert_json::<_, alloy_rpc_types::TransactionReceipt>(
            &[&tx_first_receipt],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.tx-receipt.0.json"
            ),
        );

        assert!(tx_first_kinet_logs.is_empty());

        let kinet_log = transactions
            .iter()
            .flat_map(|(_, _, logs)| logs.iter())
            .next()
            .unwrap();

        assert_json::<_, KinetNotification<alloy_rpc_types::Log>>(
            &[&kinet_log],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.kinet-log.0.json"
            ),
        );
        ...
        assert_json::<_, alloy_rpc_types::Log>(
            &[&kinet_log.data],
            include_str!(
                "../../../kinet-execution/rust/crates/kinet-exec-events/test/data/exec-events-emn-30b-15m/0.log.0.json"
            ),
        );
    }
