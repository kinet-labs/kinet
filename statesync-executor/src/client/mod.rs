...
use kinet_types::{DropTimer, NodeId, SeqNum};
...
impl<PT: PubKey> StateSyncClient<PT> {
    pub fn start(
        ...
        thread::Builder::new()
            .name("kinet-statesync".to_string())
            .spawn(move || {
                    ...
                    match response {
                        SyncResponse::UpdateTarget(target) => {
                            if current_target.is_none() {
                                ...
                                unsafe {
                                    ffi::kinet_statesync_client_handle_target(
                        ...
                        SyncResponse::Response((from, response)) => {
                            ...
                            unsafe {
                                for upsert in &response.response {
                                    let upsert_result = ffi::kinet_statesync_client_handle_upsert(
                                        ctx,
                                ...
                                if response.response_n != 0 {
                                    ffi::kinet_statesync_client_handle_done(
                                        sync_ctx.get_or_create_ctx_with_client_prefixes(),
                                        ffi::kinet_sync_done {
                                            success: true,
