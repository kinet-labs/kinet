...
use kinet_executor_glue::{
    ConfigEvent, ConfigReloadCommand, ConfigUpdate, KnownPeersUpdate, KinetEvent, PeerEntry,
    PeerEntryAddress,
};
use kinet_node_config::{NodeBootstrapPeerConfig, NodeConfig};
use kinet_types::{ExecutionProtocol, NodeId};
    ...
    SCT: SignatureCollection,
{
    ...
    fn metrics(&self) -> kinet_executor::ExecutorMetricsChain<'_> {
        Default::default()
    }
}
    ...
    EPT: ExecutionProtocol,
{
    ...
    fn poll_next(
        ...
        this.response_rx.poll_recv(cx).map(|maybe_event| {
            maybe_event.map(|config_event| KinetEvent::ConfigEvent(config_event))
        })
    }
}
