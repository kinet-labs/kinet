...
use kinet_triedb_utils::triedb_env::{BlockKey, Triedb};
...
use crate::{
    ...
    types::{
        eth_json::{
            BlockTagOrHash, BlockTags, EthAddress, EthHash, FixedData, KinetU256, Quantity,
            UnformattedData,
        },
        ethhex,
...
pub async fn kinet_debug_getRawBlock<T: Triedb>(
    data_provider: &DataProvider<T>,
    max_response_size: usize,
    params: DebugBlockParams,
) -> JsonRpcResult<String> {
    trace!("kinet_debug_getRawBlock: {params:?}");
...
pub async fn kinet_debug_getRawHeader<T: Triedb>(
    data_provider: &DataProvider<T>,
    params: DebugBlockParams,
) -> JsonRpcResult<String> {
    trace!("kinet_debug_getRawHeader: {params:?}");
...
#[serde(transparent)]
pub struct KinetDebugGetRawReceiptsResult {
    receipts: Vec<String>,
}
...
pub async fn kinet_debug_getRawReceipts<T: Triedb>(
    data_provider: &DataProvider<T>,
    max_response_size: usize,
    params: DebugBlockParams,
) -> JsonRpcResult<KinetDebugGetRawReceiptsResult> {
    trace!("kinet_debug_getRawReceipts: {params:?}");
    ...
    Ok(KinetDebugGetRawReceiptsResult { receipts })
}

#[derive(Deserialize, Debug, schemars::JsonSchema)]
pub struct KinetDebugGetRawTransactionParams {
    tx_hash: EthHash,
}
...
pub async fn kinet_debug_getRawTransaction<T: Triedb>(
    data_provider: &DataProvider<T>,
    params: KinetDebugGetRawTransactionParams,
) -> JsonRpcResult<String> {
    trace!("kinet_debug_getRawTransaction: {params:?}");
...
pub struct KinetDebugTraceTransactionParams {
    pub tx_hash: EthHash,
    #[serde(default)]
...
pub struct KinetCallFrame {
    ...
    pub calls: Vec<KinetCallFrame>,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub logs: Vec<KinetCallFrameLog>,
}

impl From<CallFrame> for KinetCallFrame {
    fn from(value: CallFrame) -> Self {
        ...
        let revert_reason = error
            .as_ref()
            .and_then(|_| kinet_ethcall::decode_revert_message(&value.output));
        ...
        Self {
            ...
            logs: value
                ...
                .into_iter()
                .enumerate()
                .map(|(i, log)| KinetCallFrameLog::from_call_frame_log(log, i))
                .collect(),
        }
    }
}
...
#[derive(Deserialize, Debug, schemars::JsonSchema)]
pub struct KinetDebugTraceBlockByHashParams {
...
pub async fn kinet_debug_traceBlockByHash<T: Triedb>(
    data_provider: &DataProvider<T>,
    max_response_size: usize,
    params: KinetDebugTraceBlockByHashParams,
) -> JsonRpcResult<Vec<KinetDebugTraceBlockResult>> {
    trace!("kinet_debugTraceBlockByHash: {params:?}");
...
#[derive(Clone, Copy, Deserialize, Debug, schemars::JsonSchema)]
pub struct KinetDebugTraceBlockByNumberParams {
...
#[derive(Serialize, Debug, schemars::JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct KinetDebugTraceBlockResult {
    pub tx_hash: EthHash,
    pub result: KinetCallFrame,
}
...
pub async fn kinet_debug_traceBlockByNumber<T: Triedb>(
    data_provider: &DataProvider<T>,
    max_response_size: usize,
    params: KinetDebugTraceBlockByNumberParams,
) -> JsonRpcResult<Vec<KinetDebugTraceBlockResult>> {
    trace!("kinet_debugTraceBlockByNumber: {params:?}");
...
pub async fn kinet_debug_traceTransaction<T: Triedb>(
    data_provider: &DataProvider<T>,
    max_response_size: usize,
    params: KinetDebugTraceTransactionParams,
) -> JsonRpcResult<Option<KinetCallFrame>> {
    trace!("kinet_eth_debugTraceTransaction: {params:?}");
...
async fn decode_block_call_frames<T: Triedb>(
    ...
    for (call_frame, tx_id) in call_frames.into_iter().zip(tx_hashes) {
        ...
        let result = KinetDebugTraceBlockResult {
...
fn build_call_tree(frames: Vec<CallFrame>) -> Option<KinetCallFrame> {
    let mut frames = frames.into_iter();

    let mut root = KinetCallFrame::from(frames.next()?);

    // Children of the root frame that have not been completely reassembled
    let mut incomplete_frames: Vec<KinetCallFrame> = Vec::new();
    ...
    for next_frame in frames.map(Some).chain(std::iter::once(None)) {
        ...
        if let Some(next_frame) = next_frame {
            incomplete_frames.push(KinetCallFrame::from(next_frame));
        }
    }

    fn extend_logs_by_emit_order<'a>(
        frame: &'a mut KinetCallFrame,
        logs_by_emit_order: &mut Vec<&'a mut KinetCallFrameLog>,
    ) {
...
mod tests {
    ...
    use kinet_archive::test_utils::mock_tx;
    use kinet_eth_types::{EthTxHash, ReceiptWithLogIndex, TransactionLocation};
    use kinet_triedb_utils::mock_triedb;
    use kinet_types::SeqNum;
    ...
    async fn test_build_call_tree() {
        ...
        let result: KinetCallFrame = result.unwrap();
    ...
    async fn debug_trace_revert() {
        ...
        let resp = kinet_debug_traceTransaction(
            &data_provider,
            usize::MAX,
    ...
    async fn debug_trace_transaction_rejects_oversized_response() {
        ...
        let data_provider = DataProvider::new(None, Arc::new(mock_triedb), None);
        let err = kinet_debug_traceTransaction(
            &data_provider,
            1,
            KinetDebugTraceTransactionParams {
                tx_hash: FixedData::<32>([0u8; 32]),
    ...
    async fn decode_block_call_frames_rejects_when_running_sum_exceeds_limit() {
        use kinet_triedb_utils::triedb_env::FinalizedBlockKey;
    ...
    async fn debug_trace_create() {
        ...
        let data_provider = DataProvider::new(None, Arc::new(mock_triedb), None);
        let resp: Option<KinetCallFrame> = kinet_debug_traceTransaction(
            &data_provider,
            usize::MAX,
            KinetDebugTraceTransactionParams {
                tx_hash: FixedData::<32>([0u8; 32]),
    ...
    async fn debug_trace_null_logs() {
        ...
        let data_provider = DataProvider::new(None, Arc::new(mock_triedb), None);
        let with_logs_resp = kinet_debug_traceTransaction(
            &data_provider,
            usize::MAX,
            KinetDebugTraceTransactionParams {
                tx_hash: FixedData::<32>([0u8; 32]),
        ...
        let no_logs_resp = kinet_debug_traceTransaction(
            &data_provider,
            usize::MAX,
            KinetDebugTraceTransactionParams {
                tx_hash: FixedData::<32>([0u8; 32]),
    ...
    async fn debug_raw_receipts() {
        ...
        let data_provider = DataProvider::new(None, Arc::new(mock_triedb), None);
        let result = kinet_debug_getRawReceipts(
            &data_provider,
            25_000_000,
            DebugBlockParams {
    ...
    async fn debug_raw_block_max_response_size_exceeded() {
        ...
        let data_provider = DataProvider::new(None, Arc::new(mock_triedb), None);
        let error = kinet_debug_getRawBlock(
            &data_provider,
            txs_payload_limit,
            DebugBlockParams {
                block: BlockTags::Number(Quantity(1)),
