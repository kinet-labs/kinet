# Kinet Archive Developer Docs

Resources:
- Ops references to self-hosted archive (mongo): https://github.com/kinet-client-docs/validator-docs/blob/main/docs/full_node/self-hosted-archive/self-hosted-archive.md
...
`kinet-archiver` accepts a `--config path/to/config.toml` flag that loads the
...
```sh
kinet-archiver --config /etc/kinet/archiver.toml --start-block 12
```
...
- **ArchiveWriter**: variant of Kinet Node that writes into ArchiveDB using the kinet-archiver binary
...
The archive system is broken apart into a core library, `kinet-archive`, and 4
scoped binaries:
- `kinet-archiver` - Reads from TrieDB or another archive, writes to durable storage
- `kinet-indexer` - Builds transaction and log indexes from archived blocks
- `kinet-archive-checker` - Validates data consistency across archive replicas
- `kinet-block-writer` - Exports blocks to filesystem with compression
