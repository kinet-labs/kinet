...
TARGET_DIR="/home/kinet/kinet/ledger/"
...
if [ -n "$NEW_FILES" ]; then
    ...
    find /home/kinet/kinet/config/forkpoint/ -type f -name "forkpoint.rlp.*" -mmin +${RETENTION_FORKPOINT} -delete 2>/dev/null
    find /home/kinet/kinet/config/forkpoint/ -type f -name "forkpoint.toml.*" -mmin +${RETENTION_FORKPOINT} -delete 2>/dev/null
    find /home/kinet/kinet/config/validators/ -type f -name "validators.toml.*" -mmin +${RETENTION_VALIDATORS} -delete 2>/dev/null
    find /home/kinet/kinet/ledger/headers -type f -mmin +${RETENTION_LEDGER} -delete 2>/dev/null
    find /home/kinet/kinet/ledger/bodies -type f -mmin +${RETENTION_LEDGER} -delete 2>/dev/null
    echo "Cleanup completed successfully"
else
    echo "No new files detected. Skipping deletion of ledger files."
fi

# When kinet-blockcapd is running, BLOCKCAPD_ROOT_DIR and BLOCKCAPD_MAX_GB
# should be set in ~kinet/.env (or otherwise somehow injected into the environment
