...
mod test {
    ...
    use futures::StreamExt;
    use kinet_chain_config::{revision::MockChainRevision, ChainConfig, MockChainConfig};
    use kinet_consensus_types::{
    ...
    use kinet_eth_block_policy::EthBlockPolicy;
    use kinet_eth_testutil::{generate_block_with_txs, make_legacy_tx, secret_to_eth_address, S1};
    use kinet_eth_txpool::{EthTxPool, EthTxPoolConfig, TrackedTxLimitsConfig};
    use kinet_eth_txpool_types::{
    ...
    use kinet_executor_glue::{MempoolEvent, KinetEvent, TxPoolCommand};
    use kinet_peer_score::{ema, StdClock};
    use kinet_testutil::signing::{node_id, MockSignatures};
    use kinet_tfm::base_fee::MIN_BASE_FEE;
    use kinet_types::{Epoch, NodeId, Round, SeqNum, GENESIS_ROUND, GENESIS_SEQ_NUM};
    use tokio::sync::mpsc;
    ...
    fn collect_forwarded_txs(
        event: KinetEvent<SignatureType, SignatureCollectionType, EthExecutionProtocol>,
    ) -> Vec<bytes::Bytes> {
        let expected_signer = secret_to_eth_address(S1);

        match event {
            KinetEvent::MempoolEvent(MempoolEvent::Proposal {
                proposed_execution_inputs,
                ..
            }) => proposed_execution_inputs
                .body
    ...
    async fn test_forwarded_ingress_drop_metrics_count_remaining_batch_once() {
        ...
        assert_eq!(
            metric_value(&metrics, "kinet.bft.txpool.forwarded_ingress_enqueued_txs",),
            2
        );
        assert_eq!(
            metric_value(&metrics, "kinet.bft.txpool.forwarded_ingress_dropped_txs",),
            2
        );
        assert_eq!(
            metric_value(&metrics, "kinet.bft.txpool.forwarded_ingress_drop_events",),
            1
        );
    }
