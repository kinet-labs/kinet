...
//! A minimal swarm builder over [`kinet_sim`]: spawn a set of nodes plus a
...
use kinet_sim::{Ctx, Handle, RunOutcome, Simulation, StepLabel, Time};

use crate::net::{Deliver, Net, Network};

/// A node that can be plugged into a simulated network.
///
/// The framework owns the node's state as a `kinet_sim` process. A message
