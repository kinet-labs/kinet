...
use kinet_dataplane::udp::DEFAULT_MTU;
...
use kinet_raptorcast::config::RaptorCastConfig;
use kinet_secp::SecpSignature;
use kinet_types::{NodeId, Round, SeqNum, Stake};
use kinet_updaters::{ledger::MockableLedger, local_router::LocalRouterConfig};
...
use crate::executor::{
    make_kinet_executor, make_kinet_state, ExecutorConfig, RouterConfig, StateConfig,
};
...
async fn main() {
    ...
    let context = args.otel_endpoint.as_ref().map(|endpoint| {
        let provider = make_provider(endpoint.to_owned(), "kinet-coordinator".to_owned());
    ...
    futures_util::future::join_all(
        ...
        .map(|(index, config)| {
            let maybe_provider = args.otel_endpoint.as_ref().map(|endpoint| {
                make_provider(
                    endpoint.to_owned(),
                    format!("kinet-testground-{:?}", &config.state_config.key.pubkey()),
                )
            });
                    ...
                    router_config: match &args.router {
                        ...
                        RouterArgs::RaptorCast => RouterConfig::RaptorCast(RaptorCastConfig {
                            ...
                            deterministic_protocol_rollout:
                                kinet_raptorcast::v1_rollout::CURRENT_STAGE,
                        }),
                    },
    ...
    <SCT as SignatureCollection>::SignatureType: Unpin,
{
    ...
    // Instantiates KinetState -> ConsensusChildState
    let (mut state, init_commands) = make_kinet_state(state_read.clone(), config.state_config);

    // The (ParentExecutor) executor instantiates embedded RaptorCast router and all other executors
    let mut executor = make_kinet_executor(index, state_read, config.executor_config);
