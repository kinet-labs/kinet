...
use kinet_executor::{ExecutorMetrics, MetricDef};
...
macro_rules! define_metric_names {
    ...
    ($vis:vis $name:ident, $transport:literal) => {
        $vis static $name: $crate::MetricNames = $crate::MetricNames {
            state_initiating_sessions: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.initiating_sessions"),
                "sessions waiting for handshake response",
            ),
            state_responding_sessions: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.responding_sessions"),
                "sessions that received init and waiting for the first transport message",
            ),
            state_transport_sessions: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.transport_sessions"),
                "fully established sessions ready for data transmission",
            ),
            state_total_sessions: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.total_sessions"),
                "sum of all session states (initiating + responding + transport)",
            ),
            state_allocated_indices: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.allocated_indices"),
                "unique 32-bit identifiers assigned to active sessions",
            ),
            state_sessions_by_public_key: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.sessions_by_public_key"),
                "lookup table mapping peer public keys to sessions",
            ),
            state_sessions_by_socket: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.sessions_by_socket"),
                "lookup table mapping socket addresses to sessions",
            ),
            state_session_index_allocated: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.session_index_allocated"),
                "lifetime allocations of session indices",
            ),
            state_session_established_initiator: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
            ...
            state_session_established_responder: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
            ...
            state_session_terminated: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.session_terminated"),
                "sessions closed due to timeout or max duration",
            ),
            state_timers_size: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.timers_size"),
                "pending timer events (keepalive, rekey, timeout)",
            ),
            state_packet_queue_size: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.packet_queue_size"),
                "outbound packets waiting to be sent",
            ),
            state_initiated_session_by_peer_size: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
            ...
            state_accepted_sessions_by_peer_size: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
            ...
            state_ip_session_counts_size: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".state.ip_session_counts_size"),
                "tracks session count per ip for rate limiting",
            ),

            filter_pass: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".filter.pass"),
                "handshake requests that passed all filters",
            ),
            filter_send_cookie: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".filter.send_cookie"),
                "cookie challenges sent (between low and high watermark)",
            ),
            filter_drop: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".filter.drop"),
                "handshake requests rejected due to rate limits",
            ),
            filter_ip_request_history_size: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".filter.ip_request_history_size"),
                "lru cache tracking recent handshake requests per ip",
            ),

            api_connect: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.connect"),
                "new outbound connection attempts",
            ),
            api_decrypt: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.decrypt"),
                "inbound data packets decrypted",
            ),
            api_encrypt_by_public_key: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.encrypt_by_public_key"),
                "outbound packets encrypted using peer public key lookup",
            ),
            api_encrypt_by_socket: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.encrypt_by_socket"),
                "outbound packets encrypted using socket address lookup",
            ),
            api_disconnect: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.disconnect"),
                "explicit session termination requests",
            ),
            api_dispatch_control: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.dispatch_control"),
                "control messages processed (handshake, cookie, keepalive)",
            ),
            api_next_packet: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.next_packet"),
                "outbound packet dequeues",
            ),
            api_tick: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".api.tick"),
                "timer processing cycles",
            ),

            dispatch_handshake_init: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".dispatch.handshake_initiation"),
                "first message of handshake sent/received",
            ),
            dispatch_handshake_response: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".dispatch.handshake_response"),
                "second message of handshake sent/received",
            ),
            dispatch_cookie_reply: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".dispatch.cookie_reply"),
                "cookie challenges sent in response to handshake init",
            ),
            dispatch_keepalive: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".dispatch.keepalive"),
                "empty data packets sent to maintain session liveness",
            ),

            error_connect: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.connect"),
                "failed connection attempts (no memory, duplicate, etc)",
            ),
            error_decrypt: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.decrypt"),
                "total decryption failures (includes all decrypt error subtypes)",
            ),
            error_decrypt_nonce_outside_window: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.decrypt.nonce_outside_window"),
                "packet counter outside replay window (too old)",
            ),
            error_decrypt_nonce_duplicate: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.decrypt.nonce_duplicate"),
                "duplicate packet counter detected (replay attack)",
            ),
            error_decrypt_mac: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.decrypt.mac"),
                "chacha20poly1305 mac authentication tag verification failed",
            ),
            error_encrypt_by_public_key: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.encrypt_by_public_key"),
                "encryption failures when looking up by public key",
            ),
            error_encrypt_by_socket: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.encrypt_by_socket"),
                "encryption failures when looking up by socket address",
            ),
            error_dispatch_control: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.dispatch_control"),
                "control message processing failures",
            ),
            error_session_exhausted: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.session_exhausted"),
                "rejected due to hitting max session limit",
            ),
            error_mac1_verification_failed: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.mac1_verification_failed"),
                "handshake mac1 authentication failed",
            ),
            error_timestamp_replay: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.timestamp_replay"),
                "handshake timestamp older than previous attempt",
            ),
            error_session_not_found: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.session_not_found"),
                "operation on non-existent session",
            ),
            error_session_index_not_found: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.session_index_not_found"),
                "data packet with unknown receiver index",
            ),
            error_handshake_init_validation: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.handshake_init_validation"),
                "malformed or invalid handshake initiation",
            ),
            error_cookie_reply: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".error.cookie_reply"),
                "cookie reply validation or generation failed",
            ),
            error_handshake_response_validation: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
            ...
            enqueued_handshake_init: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".enqueued.handshake_init"),
                "handshake initiations added to outbound queue",
            ),
            enqueued_handshake_response: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".enqueued.handshake_response"),
                "handshake responses added to outbound queue",
            ),
            enqueued_cookie_reply: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".enqueued.cookie_reply"),
                "cookie challenges added to outbound queue",
            ),
            enqueued_keepalive: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".enqueued.keepalive"),
                "keepalive packets added to outbound queue",
            ),

            rate_limit_drop: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".rate_limit.drop"),
                "handshake requests dropped due to rate limiting",
            ),
            rate_limit_connect: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".rate_limit.connect"),
                "outbound connect attempts rejected due to rate limits",
            ),

            initiator_buffered_messages: &kinet_executor::MetricDef::new(
                concat!("kinet.wireauth.", $transport, ".initiator.buffered_messages"),
                "messages buffered while waiting for handshake to complete",
            ),
            initiator_messages_sent_from_buffer: &kinet_executor::MetricDef::new(
                concat!(
                    "kinet.wireauth.",
                    $transport,
