...
use kinet_consensus_types::validator_data::ValidatorsConfig;
use kinet_node_config::{ForkpointConfig, NodeConfig, SignatureCollectionType, SignatureType};
...
#[derive(Debug, Parser)]
#[command(name = "kinet-node-config-linter", about, long_about = None)]
struct Cli {
...
fn main() {
    ...
    match config_type {
        ...
        ConfigType::Chain => {
            let _: KinetChainConfig = parse(path, contents);
        }
    }
}
