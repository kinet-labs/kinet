...
use kinet_chain_config::execution_revision::KinetExecutionRevision;
use kinet_ethcall::{
    overrides::StateOverrideSet, EthCallRequest, EthCallResult, KinetExecutor, KinetTracer,
};
use kinet_rpc_docs::rpc;
use kinet_triedb_utils::triedb_env::{
    BlockKey, FinalizedBlockKey, ProposedBlockKey, Triedb, TriedbPath,
};
use kinet_types::{BlockId, Hash, SeqNum};
...
use crate::{
    ...
    types::{
        eth_json::{BlockTagOrHash, KinetCreateAccessListResult},
        ethhex,
...
pub fn check_contract_creation_size(input: Option<&Bytes>) -> Result<(), JsonRpcError> {
    ...
    let max_code_size = KinetExecutionRevision::LATEST
...
pub struct KinetEthCallParams {
    transaction: CallRequest,
    #[serde(default)]
    block: BlockTagOrHash,
    #[schemars(skip)] // TODO: move StateOverrideSet from kinet-cxx
    #[serde(default)]
...
pub struct KinetDebugTraceCallParams {
    transaction: CallRequest,
    block: BlockTagOrHash,
    #[serde(default)]
...
#[derive(Debug, Deserialize, schemars::JsonSchema, Clone)]
pub struct KinetCreateAccessListParams {
...
#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub enum CallParams {
    Call(KinetEthCallParams),
    Trace(KinetDebugTraceCallParams),
    AccessList(KinetCreateAccessListParams),
}
        ...
        match self {
            ...
            CallParams::AccessList(p) => {
                let execution_params = EthCallExecutionParams {
                    transaction: p.transaction,
                    state_overrides: p.state_overrides,
                    tracer: KinetTracer::AccessListTracer,
                };
                (execution_params, p.block)
            }
        }
...
async fn prepare_eth_call<T: Triedb + TriedbPath>(
    ...
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    params: CallParams,
...
async fn prepare_eth_call_at_block<T: Triedb + TriedbPath>(
        ...
        .await
    {
        ...
        Err(kinet_ethcall::EthCallError::Failure {
            error_code,
            gas_used,
            gas_refund,
            ..
...
pub async fn kinet_eth_call<T: Triedb + TriedbPath>(
    data_provider: &DataProvider<T>,
    eth_call_handler_config: &EthCallHandlerConfig,
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    params: KinetEthCallParams,
) -> JsonRpcResult<String> {
    trace!("kinet_eth_call: {params:?}");
...
pub async fn kinet_debug_traceCall<T: Triedb + TriedbPath>(
    data_provider: &DataProvider<T>,
    eth_call_handler_config: &EthCallHandlerConfig,
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    max_response_size: usize,
    params: KinetDebugTraceCallParams,
) -> JsonRpcResult<Box<RawValue>> {
    debug!(?params, "kinet_debug_traceCall");

    let tracer_params = params.tracer.tracer_params;
    let tracer: KinetTracer = tracer_params.into();
    ...
    match tracer {
        ...
        KinetTracer::PreStateTracer | KinetTracer::StateDiffTracer => {
        ...
        KinetTracer::AccessListTracer => Err(JsonRpcError::invalid_params()),

        KinetTracer::NoopTracer => Ok(Box::<RawValue>::default()),
    }
}
...
#[allow(non_snake_case)]
pub async fn kinet_createAccessList<T: Triedb + TriedbPath>(
    data_provider: &DataProvider<T>,
    eth_call_handler_config: &EthCallHandlerConfig,
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    params: KinetCreateAccessListParams,
) -> JsonRpcResult<KinetCreateAccessListResult> {
    trace!("kinet_createAccessList: {params:?}");
    ...
    .await?;
    KinetCreateAccessListResult::from_follow_up_call_result(access_list, call_result)
}
...
impl KinetCreateAccessListResult {
...
pub async fn kinet_admin_ethCallStatistics(
...
mod tests {
        ...
        handlers::{
            ...
            eth::{
                call::{
                    ...
                    sender_gas_allowance, CallInput, KinetDebugTraceCallParams,
                },
                CallResult, FailureCallResult, RevertCallResult, SuccessCallResult,
            },
        },
        types::{eth_json::KinetCreateAccessListResult, jsonrpc::JsonRpcError},
    };
    ...
    fn access_list_result_records_follow_up_success() {
        ...
        let result = KinetCreateAccessListResult::from_follow_up_call_result(
            access_list.clone(),
            call_result,
        )
    ...
    fn access_list_result_preserves_other_error_as_rpc_error() {
        ...
        let err = KinetCreateAccessListResult::from_follow_up_call_result(access_list, call_result)
    ...
    fn parse_call_request_with_tracer() {
        ...
        let params: KinetDebugTraceCallParams = from_str(raw).unwrap();
    ...
    fn parse_trace_call_request_requires_block() {
        ...
        assert!(from_str::<KinetDebugTraceCallParams>(raw).is_err());
    }
