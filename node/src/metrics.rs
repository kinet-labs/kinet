...
use kinet_consensus_types::metrics::Metrics as StateMetrics;
use kinet_executor::{metric_consts, ExecutorMetrics, ExecutorMetricsChain, Gauge};
use kinet_triedb_utils::{MigrationPhase, StorageStats};
...
metric_consts! {
    pub GAUGE_TRIEDB_MIGRATION_PHASE {
        name: "kinet.triedb.migration_phase",
...
pub fn record_triedb_phase_metrics(metrics: &mut ExecutorMetrics, phase: MigrationPhase) {
    ...
    // upstream (the enum is defined in kinet-triedb).
...
metric_consts! {
    pub GAUGE_TRIEDB_DISK_CAPACITY_BYTES {
        name: "kinet.triedb.disk_capacity_bytes",
    ...
    pub GAUGE_TRIEDB_DISK_USED_BYTES {
        name: "kinet.triedb.disk_used_bytes",
...
mod migration_phase_tests {
    use kinet_triedb_utils::MigrationPhase;
...
mod storage_metrics_tests {
    use kinet_triedb_utils::StorageStats;
    ...
    fn refresh_after_registration_reaches_the_scrape() {
        ...
        assert!(
            scraped.contains("kinet_triedb_disk_capacity_bytes 1000"),
            "{scraped}"
        );
        assert!(
            scraped.contains("kinet_triedb_disk_used_bytes 700"),
            "{scraped}"
        );
    }
}
