...
fn create_manager() -> (API<TestContext>, kinet_secp::PubKey, TestContext, Config) {
    let mut rng = rng();
    let keypair = kinet_secp::KeyPair::generate(&mut rng);
...
fn test_connect_rate_limit() {
    ...
    assert!(matches!(
        err,
        kinet_wireauth::Error::ConnectRateLimited { .. }
    ));
...
fn test_too_many_accepted_sessions() {
    ...
    let mut rng = rng();
    let responder_keypair = kinet_secp::KeyPair::generate(&mut rng);
    ...
    for i in 0..10 {
        ...
        let initiator_keypair = kinet_secp::KeyPair::generate(&mut rng);
...
fn test_next_deadline_includes_filter_reset() {
    ...
    let peer_keypair = kinet_secp::KeyPair::generate(&mut rng);
...
fn test_next_deadline_returns_minimum_of_session_and_filter() {
    ...
    let peer1_keypair = kinet_secp::KeyPair::generate(&mut rng);
    ...
    let peer2_keypair = kinet_secp::KeyPair::generate(&mut rng);
...
fn test_handshake_response_address_mismatch_rejected() {
    ...
    assert!(matches!(
        err,
        kinet_wireauth::Error::HandshakeResponseAddressMismatch { expected, actual }
...
fn test_stale_handshake_response_does_not_poison_pending_initiator() {
    init_tracing();
    let config = Config::default();
    let stale_peer1_keypair = kinet_secp::KeyPair::from_ikm(b"initiator key").unwrap();
    let peer1_keypair = kinet_secp::KeyPair::from_ikm(b"initiator key").unwrap();
    ...
    assert!(matches!(err, kinet_wireauth::Error::Session(_)));
...
fn test_max_initiated_sessions_limit() {
    ...
    for i in 0..3 {
        let remote_keypair = kinet_secp::KeyPair::generate(&mut rng);
    ...
    let extra_keypair = kinet_secp::KeyPair::generate(&mut rng);
    ...
    assert!(matches!(
        err,
        kinet_wireauth::Error::TooManyInitiatedSessions { limit: 3 }
    ));
}
...
fn test_buffer_limit_per_session() {
    ...
    assert!(matches!(
        err,
        kinet_wireauth::Error::BufferLimitExceeded {
            size: 110,
            limit: 100
        }
    ));
...
fn test_buffer_message_session_not_found() {
    ...
    assert!(result.is_err());
    let err = result.unwrap_err();
    assert!(matches!(err, kinet_wireauth::Error::SessionNotFound));
}
