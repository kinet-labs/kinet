...
use kinet_executor::ExecutorMetricsChain;
use kinet_wireauth::messages::{DataPacketHeader, Packet};
...
pub struct WireAuthProtocol {
    api: kinet_wireauth::API<kinet_wireauth::StdContext, Arc<kinet_secp::KeyPair>>,
}

impl WireAuthProtocol {
    pub fn new(
        metric_names: &'static kinet_wireauth::MetricNames,
        config: kinet_wireauth::Config,
        signing_key: Arc<kinet_secp::KeyPair>,
    ) -> Self {
        let context = kinet_wireauth::StdContext::new();
        Self {
            api: kinet_wireauth::API::new(metric_names, config, signing_key, context),
        }
    }
}

impl AuthenticationProtocol for WireAuthProtocol {
    type PublicKey = kinet_secp::PubKey;
    type Error = kinet_wireauth::Error;
    type Header = kinet_wireauth::messages::DataPacketHeader;
    ...
    fn dispatch(
        ...
        match Packet::try_from(packet).map_err(kinet_wireauth::Error::from)? {
