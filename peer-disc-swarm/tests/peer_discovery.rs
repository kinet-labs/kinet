...
use kinet_transformer::{GenericTransformer, GenericTransformerPipeline, LatencyTransformer};
...
fn generate_name_record(keypair: &KeyPairType) -> KinetNameRecord<SignatureType> {
...
fn setup_keys_and_swarm_builder(
    ...
    let all_peers: BTreeMap<NodeId<PubKeyType>, KinetNameRecord<SignatureType>> = keys
        .iter()
...
fn test_update_name_record() {
    ...
    let new_name_record = KinetNameRecord {
...
fn test_validator_name_record_change_propagation_to_full_node() {
    ...
    let new_node_1_name_record = KinetNameRecord {
        name_record: new_name_record,
        signature,
    };
