...
use kinet_fair_queue::{FairQueue, FairQueueBuilder};
use kinet_peer_score::{
    ema::{ScoreProvider, ScoreReader},
    StdClock,
};
use kinet_secp::ExtractEthAddress;
use kinet_types::{LimitedVec, NodeId, MAX_FORWARDED_TXS_PER_MESSAGE};
    ...
    fn poll_next(
        ...
        loop {
            ...
            if let Some(event) = this.process_event(event) {
                return Poll::Ready(Some(KinetEvent::MempoolEvent(event)));
            }
        }
