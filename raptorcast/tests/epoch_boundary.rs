...
impl ValidatorInfo {
    ...
    fn create_name_record(&self, dp: &DataplaneHandles) -> KinetNameRecord<SecpSignature> {
...
fn spawn_validator(
    ...
    name_records: HashMap<NodeId<PubKeyType>, KinetNameRecord<SecpSignature>>,
) -> (
    ...
    tokio::task::spawn_local(async move {
        let config = kinet_raptorcast::config::RaptorCastConfig::<SecpSignature> {
            shared_key: keypair,
        ...
        let mut validator_rc = kinet_raptorcast::RaptorCast::<
            ...
            kinet_raptorcast::auth::NoopAuthProtocol<PubKeyType>,
            kinet_raptorcast::auth::NopScore<NodeId<PubKeyType>>,
        >::new(
            config,
    ...
    let publish = |from: usize, round: Round, epoch: Epoch, id: u32| {
        cmd_txs[from]
            .send(RouterCommand::Publish {
                target: kinet_types::RouterTarget::Raptorcast { round, epoch },
