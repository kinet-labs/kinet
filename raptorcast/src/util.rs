...
use kinet_types::{Epoch, NodeId, Round, RoundSpan, Stake};
use kinet_validator::{
...
pub fn compute_full_hash(bytes: &[u8]) -> kinet_crypto::hasher::Hash {
...
impl<ST: CertificateSignatureRecoverable, PD> PeerAddrLookup<CertificateSignaturePubKey<ST>>
    for std::sync::Mutex<kinet_peer_discovery::driver::PeerDiscoveryDriver<PD>>
where
    PD: kinet_peer_discovery::PeerDiscoveryAlgo<SignatureType = ST>,
{
...
impl<ST: CertificateSignatureRecoverable, PD> PeerAddrLookup<CertificateSignaturePubKey<ST>>
    for kinet_peer_discovery::driver::PeerDiscoveryDriver<PD>
where
    PD: kinet_peer_discovery::PeerDiscoveryAlgo<SignatureType = ST>,
{
    fn lookup(&self, node_id: &NodeId<CertificateSignaturePubKey<ST>>) -> Option<SocketAddr> {
...
impl Recipient<kinet_crypto::NopPubKey> {
    ...
    pub fn dummy(addr: Option<SocketAddr>) -> Self {
        ...
        let pubkey = kinet_crypto::NopPubKey::from_bytes(&bytes).expect("pubkey");
...
#[cfg(test)]
impl PeerAddrLookup<kinet_crypto::NopPubKey> for DummyPeerLookup {
    fn lookup(&self, _node_id: &NodeId<kinet_crypto::NopPubKey>) -> Option<SocketAddr> {
...
mod tests {
    ...
    fn test_auto_rebroadcast() {
        ...
        let actual: HashSet<UdpMessage<kinet_secp::PubKey>> = output.into_iter().collect();
