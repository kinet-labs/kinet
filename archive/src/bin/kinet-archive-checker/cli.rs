...
use kinet_archive::cli::ArchiveArgs;

#[derive(Debug, Parser)]
#[command(
    name = "kinet-archive-checker",
...
EXAMPLES:\n\n\
  ...
  kinet-archive-checker --bucket checker-state --region us-east-1 checker \\\n\
    --init-replicas 'aws archive-1 20,aws archive-2 20,aws archive-3 20'\n\n\
  # Inspect specific faults\n\
  kinet-archive-checker --bucket checker-state inspector list-faults\n\
  kinet-archive-checker --bucket checker-state inspector inspect-block 12345 --format all\n\n\
  # Fix faults by copying from good replicas (dry run first)\n\
  kinet-archive-checker --bucket checker-state fault-fixer\n\
  kinet-archive-checker --bucket checker-state fault-fixer --commit-changes --verify\n\n\
  # Run standalone rechecker to fix false positives (runs once and exits)\n\
  kinet-archive-checker --bucket checker-state rechecker\n\n\
  # Run rechecker in worker mode (runs periodically)\n\
  kinet-archive-checker --bucket checker-state rechecker --worker --recheck-freq-min 5\n\n\
