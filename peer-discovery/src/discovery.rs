...
use kinet_node_config::{NodeBootstrapConfig, NodeBootstrapPeerConfig};
...
use crate::{
    KinetNameRecord, KinetNameRecordWithPubkey, NameRecord, PeerDiscoveryAlgo,
...
pub struct ConnectionInfo<ST: CertificateSignatureRecoverable> {
    ...
    pub name_record: KinetNameRecord<ST>,
}
...
> {
    ...
    pub self_record: KinetNameRecord<ST>,
    ...
    pub routing_info: BTreeMap<NodeId<CertificateSignaturePubKey<ST>>, KinetNameRecord<ST>>,
...
pub struct PeerDiscoveryBuilder<ST: CertificateSignatureRecoverable> {
    ...
    pub pinned_full_nodes: BTreeSet<NodeId<CertificateSignaturePubKey<ST>>>,
    pub prioritized_full_nodes: BTreeSet<NodeId<CertificateSignaturePubKey<ST>>>,
    pub bootstrap_peers: BTreeMap<NodeId<CertificateSignaturePubKey<ST>>, KinetNameRecord<ST>>,
    pub refresh_period: Duration,
    pub request_timeout: Duration,
    pub unresponsive_prune_threshold: u32,
    pub last_participation_prune_threshold: Round,
...
impl<ST: CertificateSignatureRecoverable, C: governor::clock::Clock> PeerDiscovery<ST, C> {
    ...
    fn insert_peer_to_pending(
        ...
        name_record: KinetNameRecord<ST>,
    ...
    C: governor::clock::Clock,
{
    ...
    fn update_peers(&mut self, peers: Vec<PeerEntry<ST>>) -> Vec<PeerDiscoveryCommand<ST>> {
        ...
        for peer in peers {
            ...
            match KinetNameRecord::<ST>::try_from(&peer) {
                Ok(name_record) => {
                    if name_record
                        .all_udp_sockets()
...
mod tests {
    ...
    fn name_record_addr(record: &KinetNameRecord<SignatureType>) -> SocketAddr {
    ...
    fn generate_name_record(keypair: &KeyPairType, seq_num: u64) -> KinetNameRecord<SignatureType> {
    ...
    fn generate_dummy_name_record(keypair: &KeyPairType) -> KinetNameRecord<SignatureType> {
    ...
    fn test_tcp_lookup_uses_name_record_tcp_socket() {
        ...
        let (mut state, _clock) = generate_test_state(peer0, vec![]);
        let peer1_record = KinetNameRecord::new(
    ...
    fn test_auth_only_record_has_no_unauthenticated_udp_lookup() {
        ...
        let (mut state, _clock) = generate_test_state(peer0, vec![]);
        let name_record =
            KinetNameRecord::new(NameRecord::new(ip, tcp_port, None, auth_port, 0, 1), peer1);
    ...
    fn test_ping_record(
        ...
        let routing_info = match known_record {
            Some(record) => BTreeMap::from([(peer1_pubkey, KinetNameRecord::new(record, peer1))]),
            None => BTreeMap::new(),
        };
        ...
        let name_record = KinetNameRecord::new(incoming_record, peer1);
    ...
    fn test_update_peers_rejects_direct_udp_port_collision() {
        ...
        let peer2_record = KinetNameRecord::new(
            NameRecord::new_with_ports(
                Ipv4Addr::new(8, 8, 8, 8),
                8001,
                Some(8001),
    ...
    fn test_persist_peers() {
        ...
        use kinet_testutil::signing::create_keys as create_secp_keys;
