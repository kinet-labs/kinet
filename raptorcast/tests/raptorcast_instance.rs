...
use kinet_peer_discovery::{mock::NopDiscovery, KinetNameRecord, NameRecord};
...
pub fn set_up_test(
    ...
    let mut name_records: HashMap<NodeId<PubKeyType>, KinetNameRecord<SignatureType>> =
        HashMap::from([
...
fn test_name_record(keypair: &KeyPair, addr: SocketAddrV4) -> KinetNameRecord<SignatureType> {
    ...
    KinetNameRecord::new(name_record, keypair)
}
...
fn test_peer_maps<const N: usize>(
    ...
    HashMap<NodeId<PubKeyType>, KinetNameRecord<SignatureType>>,
) {
...
type MockRaptorCast = RaptorCast<
    ...
    kinet_raptorcast::auth::NoopAuthProtocol<CertificateSignaturePubKey<SignatureType>>,
...
fn setup_raptorcast_service(
    ...
    name_records: HashMap<NodeId<PubKeyType>, KinetNameRecord<SignatureType>>,
) -> MockRaptorCast {
    new_defaulted_raptorcast_for_tests_with_name_records::<
        SignatureType,
        MockMessage,
...
async fn raptorcast_forwards_fullnodes_group_invite_at_cold_start() {
    ...
    let prepare = kinet_raptorcast::raptorcast_secondary::group_message::PrepareGroup::<PubKeyType> {
    ...
    let router_msg: kinet_raptorcast::message::OutboundRouterMessage<MockMessage, SignatureType> =
        kinet_raptorcast::message::OutboundRouterMessage::FullNodesGroup(group_msg.clone());
...
async fn publish_to_full_nodes() {
    use kinet_types::FullnodeBroadcastMode;
...
async fn primary_raptorcast_builds_once_per_round() {
    ...
    let publish = |round: u64, id: u32| {
        vec![RouterCommand::Publish {
            target: kinet_types::RouterTarget::Raptorcast {
                epoch: Epoch(0),
                round: Round(round),
            },
...
async fn secondary_raptorcast_builds_once_per_round() {
    ...
    let send_to_group = |id: u32, round: u64, broadcast_mode: FullnodeBroadcastMode| {
        let msg_bytes = kinet_raptorcast::message::OutboundRouterMessage::<
            MockMessage,
            SignatureType,
...
async fn test_priority_messages() {
    ...
    for i in 0..MESSAGE_COUNT {
        ...
        tx_rc.exec(vec![RouterCommand::PublishWithPriority {
            target: kinet_types::RouterTarget::PointToPoint(rx_nodeid),
...
async fn test_raptorcast_forwarding_priority() {
    ...
    validator1_rc.exec(vec![RouterCommand::PublishWithPriority {
        target: kinet_types::RouterTarget::Broadcast(epoch),
        message: high_priority_msg,
        priority: UdpPriority::High,
    }]);
