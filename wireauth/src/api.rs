...
pub struct API<C: Context, K: AsRef<kinet_secp::KeyPair> = kinet_secp::KeyPair> {
    state: State,
...
impl<C: Context, K: AsRef<kinet_secp::KeyPair>> API<C, K> {
    ...
    pub fn connect(
        &mut self,
        remote_static_key: kinet_secp::PubKey,
    ...
    pub fn disconnect(&mut self, public_key: &kinet_secp::PubKey) {
    ...
    pub fn is_connected_public_key(&self, public_key: &kinet_secp::PubKey) -> bool {
...
struct CompressedPublicKey([u8; kinet_secp::COMPRESSED_PUBLIC_KEY_SIZE]);

impl From<&kinet_secp::PubKey> for CompressedPublicKey {
    fn from(pubkey: &kinet_secp::PubKey) -> Self {
