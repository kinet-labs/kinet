...
use kinet_mock_swarm::{
    ...
    transformer::{KinetMessageTransformer, KinetMessageTransformerPipeline, TwinsTransformer},
};
...
use kinet_transformer::RandLatencyTransformer;
use kinet_types::{ExecutionProtocol, NodeId, SeqNum};
use kinet_updaters::{
...
where
    ...
    S: SwarmRelation<
        ...
        Pipeline = KinetMessageTransformerPipeline<CertificateSignaturePubKey<ST>>,
        RouterScheduler = NoSerRouterScheduler<
            CertificateSignaturePubKey<ST>,
            KinetMessage<ST, SCT, EPT>,
    ...
    S::TxPoolExecutor: Default,
{
    ...
    } in nodes.into_values()
    {
        ...
        let outbound_pipeline = vec![
            KinetMessageTransformer::Twins(twins_transformer),
            KinetMessageTransformer::RandLatency(RandLatencyTransformer::new(
                rng.gen(),
    ...
    if let Some(liveness_length) = liveness {
        ...
        let pipeline = vec![
            KinetMessageTransformer::Twins(liveness_transformer),
            KinetMessageTransformer::RandLatency(RandLatencyTransformer::new(
                rng.gen(),
                Duration::from_millis(delta),
            )),
