...
use kinet_blocksync::blocksync::BlockSyncSelfRequester;
...
use kinet_consensus_state::{command::ConsensusCommand, ConsensusConfig, ConsensusStateWrapper};
...
use kinet_executor_glue::{
    ...
    MempoolEvent, KinetEvent, RouterCommand, StateSyncEvent, TimeoutVariant, TimerCommand,
    TimestampCommand, TxPoolCommand, ValSetCommand,
};
use kinet_types::{ExecutionProtocol, FullnodeBroadcastMode, NodeId, Round, RouterTarget};
use kinet_validator::{
...
use crate::{
    handle_validation_error, BlockTimestamp, ConsensusMode, KinetState, KinetVersion, Role,
    ...
    CRT: ChainRevision,
{
    pub(super) fn new(
        ...
        Self {
            consensus: &mut kinet_state.consensus,
            certificate_cache: &mut kinet_state.certificate_cache,

            metrics: &mut kinet_state.metrics,
            epoch_manager: &mut kinet_state.epoch_manager,
            block_policy: &mut kinet_state.block_policy,
            state_read: &mut kinet_state.state_read,

            val_epoch_map: &kinet_state.val_epoch_map,
            leader_election: &kinet_state.leader_election,
            version: &kinet_state.version,

            block_timestamp: &kinet_state.block_timestamp,
            block_validator: &kinet_state.block_validator,
            beneficiary: &kinet_state.beneficiary,
            nodeid: &kinet_state.nodeid,
            consensus_config: &kinet_state.consensus_config,

            keypair: &kinet_state.keypair,
            cert_keypair: &kinet_state.cert_keypair,
        }
    }
        ...
        match event {
            MempoolEvent::Proposal {
                ...
                vec![Command::RouterCommand(RouterCommand::Publish {
                    ...
                    message: VerifiedKinetMessage::Consensus(msg),
                })]
            }
                ...
                consensus
                    ...
                    .map(|target| {
                        ...
                        Command::RouterCommand(RouterCommand::PublishWithPriority {
                            ...
                            message: VerifiedKinetMessage::ForwardedTx(txs.clone()),
                            priority: kinet_types::UdpPriority::Regular,
                        })
                    })
                    .collect_vec()
            }
        ...
        match command {
            ...
            ConsensusCommand::Publish { target, message } => {
                parent_cmds.push(Command::RouterCommand(RouterCommand::PublishWithPriority {
                    target,
                    message: VerifiedKinetMessage::Consensus(message),
                    priority: kinet_types::UdpPriority::High,
                }))
            }
            ...
            ConsensusCommand::Schedule { round, duration } => {
                parent_cmds.push(Command::TimerCommand(TimerCommand::Schedule {
                    ...
                    on_timeout: KinetEvent::ConsensusEvent(ConsensusEvent::Timeout(round)),
                }))
            }
            ...
            ConsensusCommand::RequestSync(block_range) => {
                parent_cmds.push(Command::LoopbackCommand(LoopbackCommand::Forward(
                    KinetEvent::BlockSyncEvent(BlockSyncEvent::SelfRequest {
            ...
            ConsensusCommand::CancelSync(block_range) => {
                parent_cmds.push(Command::LoopbackCommand(LoopbackCommand::Forward(
                    KinetEvent::BlockSyncEvent(BlockSyncEvent::SelfCancelRequest {
                        requester: BlockSyncSelfRequester::Consensus,
                        block_range,
                    }),
                )));
            ...
            ConsensusCommand::RequestStateSync { root, high_qc } => {
                parent_cmds.push(Command::LoopbackCommand(LoopbackCommand::Forward(
                    KinetEvent::StateSyncEvent(StateSyncEvent::RequestSync { root, high_qc }),
                )));
            }
            ...
            ConsensusCommand::ScheduleVote { duration, round } => {
                parent_cmds.push(Command::TimerCommand(TimerCommand::Schedule {
                    ...
                    on_timeout: KinetEvent::ConsensusEvent(ConsensusEvent::SendVote(round)),
                }))
            }
        }
        parent_cmds
