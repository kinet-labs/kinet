...
use kinet_ethcall::{
    ...
    EthCallRequest, KinetExecutor, KinetTracer,
};
...
use crate::{
    ...
    types::{
        eth_json::{
            BlockTagOrHash, BlockTags, FillTransactionResult, KinetFeeHistory, Quantity,
            UnformattedData,
        },
...
fn into_call_result(
    result: Result<kinet_ethcall::EthCallSuccess, kinet_ethcall::EthCallError>,
) -> CallResult {
    match result {
...
async fn estimate_gas_with_builder(
    ...
    let (gas_used, gas_refund) = match eth_call_fn(&txn).await {
        ...
        CallResult::Failure(error) => match error.error_code {
            kinet_ethcall::EthCallResult::OutOfGas => {
...
pub struct KinetEthEstimateGasParams {
    tx: CallRequest,
    #[serde(default)]
...
pub async fn kinet_eth_estimateGas<T: Triedb>(
    data_provider: &DataProvider<T>,
    eth_call_handler_config: &EthCallHandlerConfig,
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    params: KinetEthEstimateGasParams,
) -> JsonRpcResult<Quantity> {
    trace!("kinet_eth_estimateGas: {params:?}");

    let KinetEthEstimateGasParams {
        mut tx,
        block,
        state_override_set,
    } = params;
...
pub struct KinetEthFillTransactionParams {
    pub tx: CallRequest,
}
...
#[allow(non_snake_case)]
pub async fn kinet_eth_fillTransaction<T: Triedb>(
    data_provider: &DataProvider<T>,
    eth_call_handler_config: &EthCallHandlerConfig,
    eth_call_executor: &KinetExecutor,
    chain_id: u64,
    params: KinetEthFillTransactionParams,
...
async fn fill_transaction_with_provider<T: Triedb>(
    ...
    trace!("kinet_eth_fillTransaction: {params:?}");
...
pub async fn kinet_eth_gasPrice<T: Triedb>(
    data_provider: &DataProvider<T>,
) -> JsonRpcResult<Quantity> {
    trace!("kinet_eth_gasPrice");
...
pub async fn kinet_eth_maxPriorityFeePerGas() -> JsonRpcResult<Quantity> {
    trace!("kinet_eth_maxPriorityFeePerGas");
...
#[derive(Deserialize, Debug, schemars::JsonSchema)]
pub struct KinetEthHistoryParams {
...
pub async fn kinet_eth_feeHistory<T: Triedb>(
    data_provider: &DataProvider<T>,
    params: KinetEthHistoryParams,
) -> JsonRpcResult<KinetFeeHistory> {
    trace!("kinet_eth_feeHistory");

    let block_count = params.block_count.0;
    match block_count {
        0 => return Ok(KinetFeeHistory(FeeHistory::default())),
        1..=1024 => (),
        _ => {
...
mod tests {
    ...
    use kinet_eth_types::{EthAccount, ReceiptWithLogIndex};
    use kinet_ethcall::EthCallResult;
    ...
    fn mock_eth_call_handler_config() -> EthCallHandlerConfig {
        EthCallHandlerConfig {
            enable_stats: false,
            pool_low: kinet_ethcall::ffi::PoolConfig {
                num_threads: 0,
                num_fibers: 0,
                timeout_sec: 0,
                queue_limit: 0,
            },
            pool_high: kinet_ethcall::ffi::PoolConfig {
                num_threads: 0,
                num_fibers: 0,
                timeout_sec: 0,
                queue_limit: 0,
            },
            pool_block: kinet_ethcall::ffi::PoolConfig {
                num_threads: 0,
                num_fibers: 0,
                timeout_sec: 0,
                queue_limit: 0,
    ...
    fn make_fill_data_provider(
        ...
        mock_triedb.set_finalized_block(
            kinet_types::SeqNum(latest_block),
    ...
    async fn test_eth_fee_history() {
        ...
        let res = kinet_eth_feeHistory(
            &data_provider,
            KinetEthHistoryParams {
