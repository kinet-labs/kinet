...
use kinet_consensus::messages::consensus_message::ProtocolMessage;
...
use kinet_types::{ExecutionProtocol, NodeId, Round};
...
impl<ST, SCT, EPT> Transformer<VerifiedKinetMessage<ST, SCT, EPT>>
    ...
    EPT: ExecutionProtocol,
{
    ...
    fn transform(
        &mut self,
        link_m: LinkMessage<CertificateSignaturePubKey<ST>, VerifiedKinetMessage<ST, SCT, EPT>>,
    ) -> TransformerStream<CertificateSignaturePubKey<ST>, VerifiedKinetMessage<ST, SCT, EPT>> {
        let should_drop = match &link_m.message {
            VerifiedKinetMessage::Consensus(consensus_msg) => {
            ...
            VerifiedKinetMessage::BlockSyncRequest(_)
            | VerifiedKinetMessage::BlockSyncResponse(_) => self.drop_block_sync,
            VerifiedKinetMessage::ForwardedTx(_) => false,
            VerifiedKinetMessage::StateSyncMessage(_) => false,
        };
    ...
    EPT: ExecutionProtocol,
{
    ...
    fn transform(
        &mut self,
        message: LinkMessage<Self::NodeIdPubKey, VerifiedKinetMessage<ST, SCT, EPT>>,
    ) -> TransformerStream<Self::NodeIdPubKey, VerifiedKinetMessage<ST, SCT, EPT>> {
        let LinkMessage {
            to,
            from,
            message,
        ...
        let capture = match &message {
            ...
            VerifiedKinetMessage::BlockSyncRequest(_)
            | VerifiedKinetMessage::BlockSyncResponse(_) => {
            ...
            VerifiedKinetMessage::ForwardedTx(_) => {
                // is this correct?
                TwinsCapture::Drop
            }
            VerifiedKinetMessage::StateSyncMessage(_) => TwinsCapture::Spread(pid),
        };
...
#[derive(Debug, Clone)]
pub enum KinetMessageTransformer<PT: PubKey> {
...
impl<ST, SCT, EPT> Transformer<VerifiedKinetMessage<ST, SCT, EPT>>
    for KinetMessageTransformer<CertificateSignaturePubKey<ST>>
where
    ST: CertificateSignatureRecoverable,
    SCT: SignatureCollection<NodeIdPubKey = CertificateSignaturePubKey<ST>>,
    EPT: ExecutionProtocol,
{
    ...
    fn transform(
        ...
        match self {
            KinetMessageTransformer::Latency(t) => t.transform(message),
            KinetMessageTransformer::XorLatency(t) => t.transform(message),
            KinetMessageTransformer::RandLatency(t) => t.transform(message),
            KinetMessageTransformer::Twins(t) => t.transform(message),
            KinetMessageTransformer::Filter(t) => t.transform(message),
            KinetMessageTransformer::Partition(t) => t.transform(message),
            KinetMessageTransformer::Drop(t) => t.transform(message),
        }
    }

    fn min_external_delay(&self) -> Option<Duration> {
        match self {
            KinetMessageTransformer::Latency(t) => {
                <LatencyTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::XorLatency(t) => {
                <XorLatencyTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::RandLatency(t) => {
                <RandLatencyTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::Twins(t) => {
                <TwinsTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::Filter(t) => {
                <FilterTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::Drop(t) => {
                <DropTransformer<Self::NodeIdPubKey> as Transformer<
                    VerifiedKinetMessage<ST, SCT, EPT>,
                >>::min_external_delay(t)
            }
            KinetMessageTransformer::Partition(t) => {
...
pub type KinetMessageTransformerPipeline<PT> = Vec<KinetMessageTransformer<PT>>;
