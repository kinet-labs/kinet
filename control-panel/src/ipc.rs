...
use kinet_executor_glue::{
    ...
    KinetEvent, ReadCommand, ReloadConfig, WriteCommand,
};
...
{
    receiver: mpsc::Receiver<KinetEvent<ST, SCT, EPT>>,
    ...
    EPT: ExecutionProtocol,
{
    ...
    async fn new_connection(
        ...
        event_channel: mpsc::Sender<KinetEvent<ST, SCT, EPT>>,
    ) {
                    ...
                    ReadCommand::GetMetrics(m) => match m {
                        GetMetrics::Request => {
                            ...
                            let Ok(_) = event_channel
                                .send(KinetEvent::ControlPanelEvent(event.clone()))
                                .await
                            else {
