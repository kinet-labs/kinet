...
use kinet_chain_config::{execution_revision::ExecutionChainParams, revision::ChainParams};
...
use kinet_eth_txpool_types::{EthTxPoolDropReason, DEFAULT_TX_PRIORITY};
use kinet_eth_types::EthExecutionProtocol;
use kinet_system_calls::{validator::SystemTransactionValidator, SYSTEM_SENDER_ETH_ADDRESS};
use kinet_tfm::base_fee::MIN_BASE_FEE;
use kinet_types::{Balance, NodeId, Nonce, SeqNum};
...
impl<PT: PubKey> PoolTx<PT> {
    ...
    pub fn has_higher_priority(&self, other: &Self, _base_fee: u64) -> bool {
        ...
        //     allow said transactions in their mempools. The Kinet client, on the other hand, only
