# kinet-sim-swarm — design

The protocol-agnostic network + swarm layer between the
[`kinet-sim`](../../kinet-sim) engine and protocol-specific simulation
...
[`kinet-sim/docs/design.md`](../../kinet-sim/docs/design.md) describes the
underlying engine.

## Position in the stack

Everything in this crate is parameterized only by a node address type `Addr`
and a wire message type `Msg`; it carries no Kinet or consensus knowledge
(dependencies: `kinet-sim`, `rand`, `rand_chacha`). Protocol-specific
harnesses layer on top — the Kinet BFT mock-swarm in this repository is one
...
The network is just another `kinet-sim` process — no dedicated framework
